...
function bindQuickCardSearch() {
  ...
  const showResults = results => {
    ...
    resultsNode.innerHTML = allResults.length ? `${setFilterMarkup(allResults,selectedSet)}${visible.map(item => {const owned=ownedCardSummary(item,state.items);return `<button class="quick-card-result" type="button" data-quick-card="${esc(item.id)}" aria-label="View ${esc(item.name)} from ${esc(item.set)}, number ${esc(item.number)}${owned.quantity?`, ${owned.quantity} already owned`:''}"><img src="${esc(item.thumb || item.image || '')}" alt="${esc(item.name)} card"><span><strong>${esc(item.name)}</strong><small>${esc(item.set || 'Set unavailable')} · ${esc(item.number || 'Number unavailable')}</small><em>${esc(item.rarity || 'Rarity unavailable')} · ${esc(languageName(item.language || language.value))} · ${esc(item.variant || 'Printing unknown')}</em>${ownedSearchStatus(item)}${matchReason(item)}</span><b>${owned.quantity?'Owned':'View'}</b></button>`;}).join('')}` : '<div class="find-empty"><strong>No matching cards</strong><span>Try fewer details, verify the language, or search the collector number by itself.</span></div>';
