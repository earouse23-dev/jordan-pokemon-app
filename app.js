import { money, calculateTotals, collectionToCsv, collectionWindow, accountBackupJson, importRecordKey, parseCollectionCsv, portfolioSnapshot, runBoundedTasks, transactionReportCsv, missingSetChecklist, isStale, localIsoDate, matchesSearch, ownedCardSummary, sameCatalogCard } from './lib/core.js';
...
import { bulkOrganizePositions, completeUnknownPurchaseLot, createAppSupabase, createImportedPosition, createPosition, createWatchlistEntry, deletePosition, deleteWatchlistEntry, loadDiagnostics, loadPortfolio, loadWatchlist, recordGradingResult, recordPurchaseLot, recordSale, remapCollectionPosition, sendMagicLink, signInWithPassword, signOut, signUpWithPassword, updatePosition, updateWatchlistEntry } from './lib/supabase-data.js';
...
function applyWorkspaceMode(mode,{announce=false}={}) {
  ...
  workspaceMode=mode;document.body.dataset.workspace=mode;
  try{localStorage.setItem('mica-workspace-mode',mode);}catch{}
  $$('[data-workspace-mode]').forEach(button=>{const active=button.dataset.workspaceMode===mode;button.classList.toggle('active',active);button.setAttribute('aria-pressed',String(active));});
  const copy={guided:{title:'Guided workspace',intro:'See what your collection may be worth, what you could realistically take home, and the clearest next step.',help:'Guided keeps essential collection decisions visible and tucks away seller reporting.'},growth:{title:'Growing seller workspace',intro:'See what needs attention, how your inventory is performing, and where prices come from.',help:'Growing seller shows grading, inventory health, cash flow, and profit tools.'},pro:{title:'Pro desk',intro:'Run valuation, inventory, grading, sales, and cash-flow decisions from one dense workspace.',help:'Pro desk keeps every tool visible and fits more inventory into each screen.'}}[mode];
...
function renderPriceEvidence(item, context) {
  ...
  const agreement=report.sourceCount===0?'No matching source':report.spreadPercent===null?'One-source reference':`${report.spreadPercent.toFixed(1)}% source spread`;
...
function bindGradingEstimator(item) {
  ...
  const acquisitionPerCard=item.uid&&item.quantity&&item.costBasis!=null?Number(item.costBasis)/Number(item.quantity):null;
...
function renderWatchlistRows() {
  ...
  $('#resultCount').textContent=`${visible.length} watched item${visible.length===1?'':'s'}`;
  ...
  $('#collectionEmptyTitle').textContent=trulyEmpty?'Your watchlist is empty':'No watched items match';
  $('#collectionEmptyCopy').textContent=trulyEmpty?'Find a card or sealed product and choose Watch to save a price target.':'Try clearing the search or changing your filters.';
  $('#firstCardGuide').classList.add('hidden');
  $('#emptyAddCard').classList.toggle('hidden',!trulyEmpty);
  $('#emptyAddCard').textContent=trulyEmpty?'Find an item to watch':'Add your first item';
...
function renderCollection() {
  ...
  const soldPositions=state.items.filter(item=>(item.transactions||[]).some(transaction=>transaction.type==='sale'));const knownRealized=soldPositions.filter(item=>item.realizedGain!=null);const realized=knownRealized.reduce((sum,item)=>sum+Number(item.realizedGain),0);
  ...
  $('#realizedGain').textContent = knownRealized.length?`${realized>=0?'+':''}${money(realized)}`:'—';$('#realizedGain').title=soldPositions.length-knownRealized.length?`${soldPositions.length-knownRealized.length} sold position${soldPositions.length-knownRealized.length===1?' has':'s have'} unknown acquisition cost`:'Realized gain from FIFO-covered sales';
  ...
  $('#forSaleCount').textContent = state.items.filter(item=>item.status==='listed').length;
  ...
  if (state.ledgerView === 'for-sale') visible = visible.filter(item => item.status === 'listed');
  if (state.setFilter) visible = visible.filter(item => item.set === state.setFilter);
...
function openCardDetail(card, preferOwned=false) {
  if (!card) return;
  const owned = preferOwned ? card : state.items.find(item => sameCatalogCard(item,card));
...
function plannedSaleBasis(item,quantity) {
  ...
  if(item.lots?.length){const allocation=allocateFifo(item.lots.map(lot=>({id:lot.id,acquiredAt:lot.fifoDate||lot.acquiredAt,quantityAcquired:lot.quantityAcquired,quantityRemaining:lot.quantityRemaining,totalCostMinor:lot.totalCost==null?null:Math.round(Number(lot.totalCost)*100),costBasisKnown:lot.costBasisKnown})),count);if(allocation.unallocatedQuantity===0)return allocation.allocatedCost;}
  return item.costBasis==null?null:Math.round(Number(item.costBasis)*count/Number(item.quantity||1)*100);
}
...
function renderBuyPlanner(item,displayPrice) {
  if(displayPrice==null)return `<section class="detail-section deal-planner"><div class="detail-section-head"><h2>What should I pay?</h2><span>Exact price required</span></div><div class="unavailable-panel">Mica needs a matching market reference before it can calculate a responsible buy ceiling.</div></section>`;
...
function bindBuyPlanner(item) {
  const root=$('.deal-planner');if(!root||!$('#buyPlanResale'))return;let latest=null;
  const values=()=>({quantity:$('#buyPlanQuantity').value,expectedResaleEach:$('#buyPlanResale').value,feePercent:$('#buyPlanFees').value,otherSellingCosts:$('#buyPlanCosts').value,targetRoiPercent:$('#buyPlanRoi').value,plannedOfferEach:$('#buyPlanOffer').value});
  const update=()=>{latest=buyOfferPlan(values());const button=$('#buyPlanWatchButton');button.disabled=!latest;if(!latest){$('#buyPlanOutput').innerHTML='<div class="unavailable-panel">Enter valid prices, costs, quantity, and target return.</div>';return;}const offerClass=latest.projectedProfitMinor===null?'':latest.projectedProfitMinor>=0?'positive':'negative';const offerRows=latest.plannedOfferTotalMinor===null?'':`<div><span>Seller ask · total</span><strong>${money(latest.plannedOfferTotalMinor/100,item.currency||'USD')}</strong></div><div class="${offerClass}"><span>Profit at that ask</span><strong>${latest.projectedProfitMinor>=0?'+':''}${money(latest.projectedProfitMinor/100,item.currency||'USD')}</strong></div><div class="${offerClass}"><span>ROI at that ask</span><strong>${latest.projectedRoiPercent===null?'—':`${latest.projectedRoiPercent>=0?'+':''}${latest.projectedRoiPercent.toFixed(1)}%`}</strong></div>`;$('#buyPlanOutput').innerHTML=`<div><span>Expected gross resale</span><strong>${money(latest.grossMinor/100,item.currency||'USD')}</strong></div><div><span>Fees + other costs</span><strong>−${money((latest.marketplaceFeesMinor+latest.otherSellingCostsMinor)/100,item.currency||'USD')}</strong></div><div class="target"><span>Maximum offer · each</span><strong>${money(latest.maxOfferEachMinor/100,item.currency||'USD')}</strong></div><div class="target"><span>Maximum offer · total</span><strong>${money(latest.maxAcquisitionMinor/100,item.currency||'USD')}</strong></div>${offerRows}`;};
  $$('#buyPlanQuantity,#buyPlanResale,#buyPlanFees,#buyPlanCosts,#buyPlanRoi,#buyPlanOffer',root).forEach(input=>input.addEventListener('input',update));
  $('#buyPlanWatchButton').addEventListener('click',()=>{if(!latest)return;openWatchlistSheet(item,matchingWatchEntry(item),{targetPrice:(latest.maxOfferEachMinor/100).toFixed(2)});});update();
}
...
function renderDetail() {
  ...
  $('#editCopyButton')?.addEventListener('click', () => openPositionEditSheet(item));
  $('#manageListingButton')?.addEventListener('click', () => openPositionEditSheet(item));
  ...
  $('#recordGradingResultButton')?.addEventListener('click',()=>openGradingResultSheet(item));
  $('#correctPositionMatchButton')?.addEventListener('click',()=>openRemapPositionSheet(item));
  ...
  $('#completePurchaseHistoryButton')?.addEventListener('click',()=>openCompletePurchaseHistorySheet(item,incompleteLot));
...
function openSealedPositionSheet(product) {
  ...
  form.addEventListener('submit',async event=>{event.preventDefault();const continueAdding=event.submitter?.value==='continue';const data=values();const breakdown=acquisitionFromTotal(data.totalAcquisitionCost,data.quantity);if(!breakdown){$('#sealedPositionError').textContent='Enter a valid total acquisition cost.';return;}if(data.transactionDate>today){$('#sealedPositionError').textContent='Acquisition dates cannot be later than today.';return;}const submits=$$('button[type="submit"]',form);submits.forEach(button=>button.disabled=true);$('#sealedPositionError').textContent='Saving securely…';let itemId;try{itemId=await createPosition(supabase,{...breakdown,identity:identitySnapshot(product,'Sealed product'),cardId:null,variantId:null,cardState:'sealed',rawCondition:null,grader:null,grade:null,certificationNumber:null,quantity:Number(data.quantity),transactionDate:data.transactionDate,currency:'USD',notes:data.notes||null,idempotencyKey:crypto.randomUUID()});if(data.location)await updatePosition(supabase,itemId,{location:data.location});}catch(error){submits.forEach(button=>button.disabled=false);$('#sealedPositionError').textContent=`Could not add this product: ${error.message||'Unknown error'}`;return;}closeSheet({discardHistory:true});if(continueAdding){openSealedSearch();toast('Saved · find the next sealed product');try{await reloadPortfolio();}catch{toast('Saved · library refresh will retry automatically');}}else{toast('Sealed product added to your library');try{await reloadPortfolio(itemId);}catch{routeTo('collection');toast('Saved · library refresh will retry automatically');}}});
}
...
function openWatchlistSheet(card, existing=null, defaults={}) {
  ...
  const context=existing?`<div class="simple-note"><strong>${esc(existing.variant)} · ${esc(watchContextLabel(existing))}</strong><br>The exact context stays fixed so target comparisons remain consistent.</div>`:sealed?`<input type="hidden" name="cardState" value="sealed"><input type="hidden" name="variant" value="${esc(card.variant||'Sealed product')}"><div class="simple-note"><strong>Exact sealed product · ${esc(languageName(card.language||'en'))}</strong><br>Mica will track this provider product ID without substituting a card or another sealed product.</div>`:`<div class="form-grid">
  ...
  const syncState=()=>{if(existing||sealed)return;const graded=$('#watchState').value==='graded';$$('.graded-watch',form).forEach(node=>node.hidden=!graded);$$('.raw-watch',form).forEach(node=>node.hidden=graded);$('#watchGrader').required=graded;$('#watchGrade').required=graded;$('#watchCondition').required=!graded;};
  ...
  form.addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(form).entries());const targetPrice=data.targetPrice===''?null:Number(data.targetPrice);if(targetPrice!==null&&(!Number.isFinite(targetPrice)||targetPrice<0)){ $('#watchError').textContent='Enter a valid target price or leave it blank.';return;}const submit=form.querySelector('[type="submit"]');submit.disabled=true;$('#watchError').textContent='Saving securely…';try{
      ...
      else {const cardState=data.cardState;const rawCondition=cardState==='raw'?normalizeRawCondition(data.rawCondition).normalized:null;const grader=cardState==='graded'?normalizeGrader(data.grader).normalized:null;const grade=cardState==='graded'?normalizeGrade(data.grade):null;if(cardState==='graded'&&(!grader||!grade)){throw new Error('Choose a grading company and a grade from 1 to 10.');}const condition=cardState==='raw'?String(data.rawCondition).split('_').map(part=>part[0].toUpperCase()+part.slice(1)).join(' '):cardState==='graded'?'Graded':null;const quote=selectReferenceQuote(card.quotes,data.variant,'USD',condition?{condition,gradingCompany:grader||'',grade:grade||''}:{});const added=await createWatchlistEntry(supabase,{userId:state.session.user.id,cardId:cardState==='sealed'?null:card.cardId||null,identity:identitySnapshot(card,data.variant),cardState,rawCondition,grader,grade,targetPrice,startingMarketPrice:quote?.amount??null,currency:'USD',notes:data.notes});state.watchlist.unshift({...added,currentPrice:quote?.amount??null,quotes:card.quotes||[],pricingStatus:quote?quoteStatus(quote):'loading',pricingUpdatedAt:quote?.observedAt||null});}
    ...
    }catch(error){submit.disabled=false;$('#watchError').textContent=error.message?.includes('duplicate')||error.code==='23505'?'This exact item is already on your Watchlist.':`Could not save this watch: ${error.message||'Unknown error'}`;}});
}

function openPositionSheet(card, options={}) {
  if(card.cardState==='sealed'||card.productType){openSealedPositionSheet(card);return;}
  const today=localIsoDate();const variants=Array.isArray(card.variants)&&card.variants.length?card.variants:[card.variant||'Unknown'];
  ...
  form.addEventListener('submit',async event=>{event.preventDefault();const continueAdding=event.submitter?.value==='continue';const formInput=values();const breakdown=acquisitionFromTotal(formInput.totalAcquisitionCost,formInput.quantity);if(!breakdown){$('#positionError').textContent='Enter a valid total acquisition cost.';return;}const input={...formInput,...breakdown,quantity:Number(formInput.quantity)};delete input.saveMode;input.grade=input.cardState==='graded'?normalizeGrade(input.grade):null;input.grader=input.cardState==='graded'?normalizeGrader(input.grader).normalized:null;const validation=validateAcquisition(input,today);if(!validation.valid){$('#positionError').textContent=Object.values(validation.errors)[0];return;}const submits=$$('button[type="submit"]',form);submits.forEach(button=>button.disabled=true);$('#positionError').textContent='Saving securely…';let itemId;try{itemId=await createPosition(supabase,{...input,identity:identitySnapshot(card,input.variant),cardId:card.cardId||null,variantId:card.variantId||null,idempotencyKey:crypto.randomUUID(),currency:'USD'});}catch(error){$('#positionError').textContent=error.message?.includes('future')?'Acquisition dates cannot be later than today.':`Could not add this card: ${error.message||'Unknown error'}`;submits.forEach(button=>button.disabled=false);return;}if(options.queueEntryId){state.intakeQueue=state.intakeQueue.filter(entry=>entry.queueEntryId!==options.queueEntryId);renderIntakeQueueBar();}closeSheet({discardHistory:true});if(continueAdding){try{await reloadPortfolio();}catch{toast('Saved · library refresh will retry automatically');}if(options.queueEntryId&&state.intakeQueue.length){openIntakeQueueSheet();toast('Saved · choose the next queued card');}else{routeTo('scan');const search=$('#quickCardSearch');search.focus();search.select();toast(options.queueEntryId?'Queue complete · search for more':'Saved · search for the next card');}}else{toast('Added to your library');try{await reloadPortfolio(itemId);}catch{routeTo('collection');toast('Saved · library refresh will retry automatically');}}});
}
...
function openCompletePurchaseHistorySheet(item,lot) {
  if(!lot)return;
  const today=localIsoDate();
  const needsCost=!lot.costBasisKnown;const needsDate=!lot.acquisitionDateKnown;
  ...
  $('#completePurchaseHistoryForm').addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget).entries());const totalAcquisitionCost=needsCost&&data.totalAcquisitionCost!==''?Number(data.totalAcquisitionCost):null;const acquiredAt=needsDate&&data.acquiredAt?data.acquiredAt:null;if(totalAcquisitionCost===null&&!acquiredAt){$('#completePurchaseHistoryError').textContent='Add a total acquisition cost or the original purchase date.';return;}if(totalAcquisitionCost!==null&&(!Number.isFinite(totalAcquisitionCost)||totalAcquisitionCost<0)){ $('#completePurchaseHistoryError').textContent='Enter a valid total acquisition cost.';return;}if(acquiredAt&&acquiredAt>today){$('#completePurchaseHistoryError').textContent='Acquisition dates cannot be later than today.';return;}const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;$('#completePurchaseHistoryError').textContent='Updating purchase history…';try{await completeUnknownPurchaseLot(supabase,{purchaseLotId:lot.id,totalAcquisitionCost,acquiredAt});closeSheet({discardHistory:true});await reloadPortfolio(item.uid);toast('Purchase history updated');}catch(error){const message=String(error.message||'');$('#completePurchaseHistoryError').textContent=message.includes('already_known')?'This purchase was already updated. Refresh and try again.':message.includes('future')?'Acquisition dates cannot be later than today.':`Could not update this purchase: ${message||'Unknown error'}`;submit.disabled=false;}});
}

function openGradingResultSheet(item) {
  const today=localIsoDate();
  const latestAcquisition=(item.lots||[]).map(lot=>lot.acquiredAt).filter(Boolean).sort().at(-1)||'';
  const basisReady=(item.lots||[]).length>0&&(item.lots||[]).every(lot=>lot.costBasisKnown);
  const batchNote=Number(item.quantity)>1?'Every copy in this position must have the same grader and grade. Leave certification blank unless one number represents the whole position. Use separate positions when results differ.':'The original purchase stays in place; this adds only the grading cost and returned slab details.';
  openSheet(`<div class="sheet-heading"><div><h2 id="sheetTitle">Record returned grade</h2><p>${esc(item.name)} · convert this raw position without a fake sale</p></div><button class="sheet-close" aria-label="Close">×</button></div><form id="gradingResultForm"><div class="form-grid">
    <div class="field"><label for="gradingResultGrader">Grading company</label><select id="gradingResultGrader" name="grader" required><option value="">Choose grader</option>${['PSA','BGS','CGC','TAG','SGC'].map(value=>`<option>${value}</option>`).join('')}</select></div>
    ...
    <div class="field"><label for="gradingResultDate">Return date</label><input id="gradingResultDate" name="transactionDate" type="date" min="${esc(latestAcquisition)}" max="${today}" value="${today}" required></div>
  ...
  $('#gradingResultForm').addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget).entries());const normalizedGrade=normalizeGrade(data.grade);const normalizedGrader=normalizeGrader(data.grader).normalized;const total=Number(data.totalGradingCost);if(!normalizedGrader||!normalizedGrade){$('#gradingResultError').textContent='Choose a supported grading company and enter a grade from 1 to 10.';return;}if(!Number.isFinite(total)||total<0){$('#gradingResultError').textContent='Enter the total all-in grading cost.';return;}if(data.transactionDate>today||(latestAcquisition&&data.transactionDate<latestAcquisition)){$('#gradingResultError').textContent='The return date must be after the known purchase dates and cannot be in the future.';return;}const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;$('#gradingResultError').textContent='Preserving the ledger and updating this position…';try{await recordGradingResult(supabase,{...data,collectionItemId:item.uid,grader:normalizedGrader,grade:normalizedGrade,totalGradingCost:total,idempotencyKey:crypto.randomUUID()});closeSheet({discardHistory:true});await reloadPortfolio(item.uid);toast('Returned grade recorded · original purchase history preserved');}catch(error){const message=String(error.message||'');$('#gradingResultError').textContent=message.includes('acquisition_cost_required')?'Complete the missing acquisition cost first.':message.includes('position_not_raw')?'This position is already graded.':message.includes('position_not_owned')?'Return this position to owned status before recording grading.':message.includes('grading_before_acquisition')?'The grading date cannot be before a known purchase date.':`Could not record the graded result: ${message||'Unknown error'}`;submit.disabled=false;}});
}
...
function openPositionEditSheet(item) {
  const favorite=(item.tags||[]).some(tag=>String(tag).toLowerCase()==='favorites');const labels=(item.tags||[]).filter(tag=>String(tag).toLowerCase()!=='favorites');const today=localIsoDate();const editableStatus=['owned','listed','archived'].includes(item.status)?item.status:'owned';
  openSheet(`<div class="sheet-heading"><div><h2 id="sheetTitle">Edit position details</h2><p>${esc(item.name)} · financial transactions remain auditable</p></div><button class="sheet-close" aria-label="Close">×</button></div><form id="positionEditForm"><div class="form-grid"><div class="field full"><label for="editStatus">What are you doing with it?</label><select id="editStatus" name="status"><option value="owned" ${editableStatus==='owned'?'selected':''}>Keeping it</option><option value="listed" ${editableStatus==='listed'?'selected':''}>Listed for sale</option><option value="archived" ${editableStatus==='archived'?'selected':''}>Archived</option></select><small>Choose Listed for sale to open seller details. A recorded sale remains a separate auditable transaction.</small></div><div class="listing-edit-fields full" id="listingEditFields"><div class="form-grid"><div class="field"><label for="editAskingPrice">Asking price · each</label><div class="money-input"><span>$</span><input id="editAskingPrice" name="askingPrice" type="number" inputmode="decimal" min="0" step="0.01" value="${item.askingPrice??''}" placeholder="0.00"></div></div><div class="field"><label for="editListedAt">Listed date</label><input id="editListedAt" name="listedAt" type="date" max="${today}" value="${esc(item.listedAt||today)}"></div><div class="field full"><label for="editListingVenue">Where is it listed?</label><input id="editListingVenue" name="listingVenue" maxlength="100" value="${esc(item.listingVenue||'')}" placeholder="eBay, TCGplayer, card show table…"></div></div><p>Mica compares your ask with the exact current market and flags it for another review after 7 days.</p></div><div class="field full"><label for="editCertification">Certification number</label><input id="editCertification" name="certificationNumber" value="${esc(item.certificationNumber||'')}"></div><div class="field full"><label for="editLocation">Storage location</label><input id="editLocation" name="location" maxlength="250" value="${esc(item.location||'')}" placeholder="Binder 1 · Page 4"></div><div class="field full"><label for="editTags">Labels <span class="optional-label">Optional</span></label><input id="editTags" name="tags" maxlength="500" value="${esc(labels.join(', '))}" placeholder="Trade binder, Grade next, Show case"><small>Separate labels with commas. Favorites is managed from the card page.</small></div><div class="field full"><label for="editNotes">Notes</label><textarea id="editNotes" name="notes" maxlength="10000">${esc(item.notes||'')}</textarea></div><p class="form-error" id="editError" role="alert"></p></div><div class="sheet-actions"><button class="secondary" type="button" id="editCancel">Cancel</button><button class="primary" type="submit">Save details</button></div></form>`);
  const syncListing=()=>{const listed=$('#editStatus').value==='listed';$('#listingEditFields').hidden=!listed;$('#editAskingPrice').required=listed;$('#editListingVenue').required=listed;$('#editListedAt').required=listed;};
  $('#editStatus').addEventListener('change',syncListing);syncListing();$('#editCancel').addEventListener('click',closeSheet);$('#positionEditForm').addEventListener('submit',async event=>{event.preventDefault();const data=Object.fromEntries(new FormData(event.currentTarget).entries());const tags=[...new Map(String(data.tags||'').split(',').map(tag=>tag.trim()).filter(Boolean).map(tag=>[tag.toLowerCase(),tag])).values()].slice(0,favorite?19:20);if(tags.some(tag=>tag.length>40)){ $('#editError').textContent='Keep each label to 40 characters or fewer.';return;}if(data.status==='listed'&&(data.askingPrice===''||Number(data.askingPrice)<0||!data.listingVenue.trim())){$('#editError').textContent='Add an asking price and listing venue so this listing is ready to manage.';return;}if(data.status==='listed'&&data.listedAt>today){$('#editError').textContent='The listed date cannot be later than today.';return;}if(favorite)tags.unshift('Favorites');delete data.tags;const listing=data.status==='listed';const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;try{await updatePosition(supabase,item.uid,{...data,tags,askingPrice:listing?data.askingPrice:null,listingVenue:listing?data.listingVenue:null,listedAt:listing?(data.listedAt||today):null,priceReviewedAt:listing?today:null});closeSheet({discardHistory:true});await reloadPortfolio(item.uid);toast(listing?'Listing saved and price marked reviewed':'Position details updated');}catch(error){$('#editError').textContent=`Could not update position: ${error.message||'Unknown error'}`;submit.disabled=false;}});
}

function catalogVariants(card) {
  const values=(card.variants||[]).map(value=>String(value));const normalized=values.map(value=>normalizeIdentity(value));const first=normalized.some(value=>value.includes('firstedition'));const holo=normalized.some(value=>value.includes('holo')&&!value.includes('reverse'));const reverse=normalized.some(value=>value.includes('reverse'));const normal=normalized.some(value=>value.includes('normal'));
  const result=[];if(first&&holo)result.push('1st Edition Holofoil');if(first&&(normal||!holo))result.push('1st Edition Normal');if(holo)result.push('Holofoil');if(reverse)result.push('Reverse Holofoil');if(normal)result.push('Normal');values.forEach((value,index)=>{if(!/(firstedition|holo|reverse|normal)/.test(normalized[index]))result.push(value.replace(/([a-z])([A-Z])/g,'$1 $2').replace(/^./,letter=>letter.toUpperCase()));});if(!result.length&&card.variant)result.push(card.variant);return [...new Set(result.length?result:['Unknown'])];
}

function openRemapPositionSheet(item) {
  let selected=null;let requestId=0;let timer;
  ...
  const renderResults=cards=>{selected=null;$('#remapSave').disabled=true;$('#remapChoice').hidden=true;$('#remapResults').innerHTML=cards.length?cards.map((card,index)=>`<button class="catalog-result" type="button" data-remap-result="${index}"><img src="${esc(card.thumb||card.image||'./icons/icon.svg')}" alt=""><span><strong>${esc(card.name)}</strong>${esc(card.set)} · ${esc(card.number)}<small>${esc(languageName(card.language))} · ${esc(card.rarity||'Rarity unavailable')}</small>${matchReason(card)}</span><b>Select</b></button>`).join(''):'<div class="unavailable-panel">No exact catalog matches found. Try fewer details or verify the language.</div>';$$('[data-remap-result]',$('#remapResults')).forEach(button=>button.addEventListener('click',()=>{selected=cards[Number(button.dataset.remapResult)];const variants=catalogVariants(selected);$('#remapChoice').hidden=false;$('#remapChoice').innerHTML=`<strong>Selected: ${esc(selected.name)} · ${esc(selected.set)} ${esc(selected.number)}</strong><br><label for="remapVariant">Exact printing</label><select id="remapVariant">${variants.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('')}</select>`;$('#remapSave').disabled=false;$$('[data-remap-result]',$('#remapResults')).forEach(row=>row.setAttribute('aria-pressed',String(row===button)));}));};
  const search=async()=>{const query=$('#remapQuery').value.trim();const current=++requestId;if(query.length<2){$('#remapResults').innerHTML='<div class="unavailable-panel">Type at least two characters.</div>';return;}$('#remapResults').setAttribute('aria-busy','true');$('#remapResults').innerHTML='<div class="searching-cards"><i></i><span>Finding exact printings…</span></div>';try{const result=await searchCatalog(query,$('#remapLanguage').value,12);if(current!==requestId)return;renderResults(result.items);}catch{if(current===requestId)$('#remapResults').innerHTML='<div class="unavailable-panel">Catalog search is temporarily unavailable. Your position is unchanged.</div>';}finally{if(current===requestId)$('#remapResults').setAttribute('aria-busy','false');}};
  $('#remapQuery').addEventListener('input',()=>{clearTimeout(timer);timer=setTimeout(search,250);});$('#remapLanguage').addEventListener('change',search);$('#remapCancel').addEventListener('click',closeSheet);$('#remapPositionForm').addEventListener('submit',async event=>{event.preventDefault();if(!selected||!$('#remapVariant'))return;const submit=$('#remapSave');submit.disabled=true;$('#remapError').textContent='Updating the exact match…';const variant=$('#remapVariant').value;try{await remapCollectionPosition(supabase,{collectionItemId:item.uid,identity:identitySnapshot(selected,variant),cardId:selected.cardId||null,variantId:selected.variantId||null});closeSheet({discardHistory:true});await reloadPortfolio(item.uid);toast('Catalog match corrected · pricing history restarted');}catch(error){submit.disabled=false;$('#remapError').textContent=`Could not correct this match: ${error.message||'Unknown error'}`;}});void search();
}
...
function ownedSearchStatus(item) {
  const owned=ownedCardSummary(item,state.items);if(!owned.quantity)return '';
...
function openManualSearch() {
  ...
  const bindResults=()=>{ const visible=selectedSet?allResults.filter(item=>item.set===selectedSet):allResults; $('#manualResults').innerHTML=allResults.length?`${setFilterMarkup(allResults,selectedSet)}${visible.map(item=>{const owned=ownedCardSummary(item,state.items);return `<button class="catalog-result" type="button" data-catalog-id="${esc(item.id)}" aria-label="Review ${esc(item.name)} from ${esc(item.set)}, number ${esc(item.number)}${owned.quantity?`, ${owned.quantity} already owned`:''}"><img src="${esc(item.thumb||'')}" alt="${esc(item.name)} card"><span><strong>${esc(item.name)}</strong>${esc(item.set||'Set unavailable')} · ${esc(item.number)}<small>${esc(item.rarity||'Rarity unavailable')} · ${esc(languageName(item.language || language.value))} · ${esc(item.variant)}</small>${ownedSearchStatus(item)}${matchReason(item)}</span><b>${owned.quantity?'Owned':'Review'}</b></button>`;}).join('')}`:'<div class="unavailable-panel">No catalog matches found. Try fewer details or verify the language.</div>'; $$('[data-result-set]', $('#manualResults')).forEach(button=>button.addEventListener('click',()=>{selectedSet=button.dataset.resultSet;bindResults();})); $$('[data-catalog-id]', $('#manualResults')).forEach(button=>button.addEventListener('click',()=>{const card=catalog.find(item=>item.id===button.dataset.catalogId);closeSheet({discardHistory:true});openCardDetail(card);})); };
...
function downloadCollectionCsv() {
  ...
  const date=localIsoDate();downloadTextFile(collectionToCsv(state.items),'text/csv;charset=utf-8',`mica-collection-${date}.csv`);toast('Importable collection CSV downloaded');
  return true;
}
...
function handleCsv(file) {
  ...
  const reader=new FileReader();reader.onerror=()=>toast('Mica could not read that CSV');reader.onload=()=>{
    const {records,errors,source='Generic CSV'}=parseCollectionCsv(String(reader.result));
    ...
    const prepare=(record,index)=>{
      const cardState=record.cardState==='sealed'?'sealed':record.cardState==='graded'||Boolean(record.gradingCompany)?'graded':'raw';const rawCondition=cardState==='raw'?normalizeRawCondition(record.rawCondition||record.condition).normalized:null;const grader=cardState==='graded'?normalizeGrader(record.gradingCompany).normalized:null;const grade=cardState==='graded'?normalizeGrade(record.grade):null;const acquisitionDateKnown=Boolean(record.purchaseDate);const transactionDate=record.purchaseDate||$('#importFallbackDate').value;const total=record.totalAcquisitionCost??(record.cost===null?null:Number(record.cost)*Number(record.quantity));const acquisitionCostKnown=total!==null;
      if(acquisitionCostKnown&&(!Number.isFinite(Number(total))||Number(total)<0))return {error:`Row ${index+2}: purchase price or total acquisition cost is invalid`};
      ...
      const exact=cardState==='sealed'?null:catalog.find(item=>item.id===record.id)||(record.externalIds?.tcgplayer?catalog.find(item=>String(item.externalIds?.tcgplayer||'')===String(record.externalIds.tcgplayer)):null)||(catalog.find(item=>normalizeIdentity(item.name)===normalizeIdentity(record.name)&&normalizeIdentity(item.set)===normalizeIdentity(record.set)&&normalizeIdentity(item.number)===normalizeIdentity(record.number)));const sealedId=cardState==='sealed'?String(record.id||'').match(/^sealed:(\d{1,12})$/)?.[1]||null:null;const card={...record,...exact,id:exact?.id||record.id||`import:${normalizeIdentity(`${record.name}-${record.set}-${record.number}`)||'card'}`,language:record.language||exact?.language||'en',variant:record.variant||exact?.variant||(cardState==='sealed'?'Sealed':'Unknown'),productType:cardState==='sealed'?(record.productType||'sealed'):record.productType||exact?.productType||null,externalIds:cardState==='sealed'&&sealedId?{pkmnpricesSealed:Number(sealedId)}:{...(exact?.externalIds||{}),...(record.externalIds||{})}};const breakdown=acquisitionFromTotal(acquisitionCostKnown?total:0,record.quantity);
      const currency=/^[A-Z]{3}$/.test(record.currency||'USD')?record.currency:'USD';
      return {record,card,keyRecord:{...record,id:card.id,language:card.language,variant:card.variant,cardState,rawCondition,gradingCompany:grader,grade,quantity:Number(record.quantity),totalAcquisitionCost:acquisitionCostKnown?Number(total):null,purchaseDate:transactionDate,currency},input:{...breakdown,identity:{...identitySnapshot(card,card.variant),acquisitionCostKnown,acquisitionDateKnown},cardId:exact?.cardId||null,variantId:exact?.variantId||null,cardState,rawCondition,grader,grade,certificationNumber:record.certificationNumber||null,quantity:Number(record.quantity),transactionDate,currency,notes:record.notes||null}};
    };
    let entries=[];let validationFailures=[];let pauseRequested=false;let running=false;
    const setRunning=value=>{running=value;$('#bottomSheet').dataset.lockClose=value?'true':'false';$('#importFallbackDate').disabled=value;$('#cancelCsvImport').disabled=value;$('.sheet-close').disabled=value;$('#pauseCsvImport').hidden=!value;$('#addCsvImport').disabled=value;};
    const updateProgress=active=>{const settled=active?entries.filter(item=>item.status==='done').length+active.completed:entries.filter(item=>item.status==='done'||item.status==='failed').length;const finished=active?entries.filter(item=>item.status==='done').length+active.succeeded:entries.filter(item=>item.status==='done').length;const failed=active?active.failed:entries.filter(item=>item.status==='failed').length;const remaining=Math.max(0,entries.length-settled);$('#importProgress').hidden=false;$('#importProgressBar').max=Math.max(entries.length,1);$('#importProgressBar').value=settled;$('#importProgressCount').textContent=`${settled.toLocaleString()} of ${entries.length.toLocaleString()}`;$('#importProgressTitle').textContent=running?(pauseRequested?'Finishing current secure saves…':'Saving securely…'):(remaining?'Import paused':'Import complete');$('#importProgressHelp').textContent=running?`${finished.toLocaleString()} saved · ${failed.toLocaleString()} failed so far. Pause finishes current saves first.`:remaining?`${remaining.toLocaleString()} row${remaining===1?'':'s'} ready to continue.`:`${finished.toLocaleString()} saved or safely recovered · ${failed.toLocaleString()} failed.`;};
    const finishView=async()=>{try{await reloadPortfolio();}catch(error){validationFailures.push(`Portfolio refresh: ${error.message||'try refreshing the page'}`);}state.ledgerView='all';state.query='';state.setFilter='';state.conditionFilter='';state.labelFilter='';$('#collectionSearch').value='';syncTabs();renderCollection();};
    const runImport=async()=>{
      if(running)return;const pending=entries.filter(item=>item.status==='pending'||item.status==='failed');if(!pending.length)return;pending.forEach(item=>{item.status='pending';delete item.error;});pauseRequested=false;setRunning(true);updateProgress();
      const result=await runBoundedTasks(pending,async item=>{const saved=await createImportedPosition(supabase,{...item.input,idempotencyKey:item.idempotencyKey});if(item.record.location||(item.record.tags||[]).length)await updatePosition(supabase,saved.id,{location:item.record.location||'',tags:(item.record.tags||[]).slice(0,50)});return saved;},{concurrency:4,shouldStop:()=>pauseRequested,onProgress:updateProgress});
      result.results.forEach((outcome,index)=>{const item=pending[index];if(!outcome)return;item.status=outcome.status==='fulfilled'?'done':'failed';if(outcome.status==='rejected')item.error=`${item.record.name}: ${outcome.reason?.message||'could not save'}`;});
      $('#pauseCsvImport').disabled=true;$('#importProgressTitle').textContent='Refreshing your collection…';$('#importProgressHelp').textContent='Your saved rows are safe. Mica is loading the updated library.';await finishView();setRunning(false);updateProgress();const remaining=entries.filter(item=>item.status==='pending').length;const failed=entries.filter(item=>item.status==='failed');const done=entries.filter(item=>item.status==='done').length;$('#cancelCsvImport').textContent='Close';$('#addCsvImport').disabled=false;$('#addCsvImport').textContent=remaining?'Continue import':failed.length?'Retry failed rows':'Import complete';$('#addCsvImport').disabled=!remaining&&!failed.length;const issues=[...validationFailures,...failed.map(item=>item.error)].filter(Boolean);$('#importStatus').textContent=remaining?`${done.toLocaleString()} saved · paused with ${remaining.toLocaleString()} remaining.`:issues.length?`${done.toLocaleString()} saved · ${issues.length.toLocaleString()} issue${issues.length===1?'':'s'}. ${issues.slice(0,3).join(' · ')}`:`${done.toLocaleString()} position${done===1?'':'s'} saved. Safe to close.`;toast(remaining?`Import paused · ${done.toLocaleString()} saved`:issues.length?`${done.toLocaleString()} saved · ${issues.length.toLocaleString()} need review`:`${done.toLocaleString()} positions saved to your account`);
    };
    $('#cancelCsvImport').addEventListener('click',closeSheet);$('#pauseCsvImport').addEventListener('click',()=>{pauseRequested=true;$('#pauseCsvImport').disabled=true;updateProgress();});$('#addCsvImport').addEventListener('click',async()=>{
      const fallback=$('#importFallbackDate');if(!fallback.reportValidity())return;
      if(!entries.length){const button=$('#addCsvImport');button.disabled=true;fallback.disabled=true;$('#importProgress').hidden=false;$('#importProgressTitle').textContent='Preparing secure row keys…';$('#importProgressHelp').textContent='Checking exact identity and duplicate rows before anything is saved.';const prepared=records.map(prepare);validationFailures=prepared.filter(item=>item.error).map(item=>item.error);const ready=prepared.filter(item=>!item.error);$('#importProgressBar').max=Math.max(ready.length,1);if(!ready.length){button.disabled=false;fallback.disabled=false;$('#importStatus').textContent=validationFailures.slice(0,3).join(' · ');return;}const occurrences=new Map();try{for(const [index,item] of ready.entries()){const baseKey=await importRecordKey(item.keyRecord);const occurrence=occurrences.get(baseKey)||0;occurrences.set(baseKey,occurrence+1);item.idempotencyKey=await importRecordKey(item.keyRecord,occurrence);item.status='pending';if(index%25===0||index===ready.length-1){$('#importProgressBar').value=index+1;$('#importProgressCount').textContent=`${(index+1).toLocaleString()} of ${ready.length.toLocaleString()}`;}}}catch{button.disabled=false;fallback.disabled=false;$('#importStatus').textContent='This browser could not prepare secure import keys. Update the browser and try again.';return;}entries=ready;if(validationFailures.length)$('#importStatus').textContent=`${validationFailures.length} invalid row${validationFailures.length===1?'':'s'} will be skipped.`;}
      $('#pauseCsvImport').disabled=false;await runImport();
    });
...
async function refreshWatchlistPricing() {
  ...
  const cardItems=unique.filter(item=>item.cardState!=='sealed');const sealedItems=unique.filter(item=>item.cardState==='sealed');
  const lookups=cardItems.map(item=>({clientId:item.id,pkmnpricesId:item.externalIds?.pkmnprices||'',justtcgId:item.externalIds?.justtcg||'',tcgplayerId:item.externalIds?.tcgplayer||'',tcgdexId:item.externalIds?.tcgdex||'',name:item.name,set:item.set,number:item.number,language:item.language||'en'}));
  try{
    const cards=new Map();const sealedProducts=new Map();const processed=new Set();const sealedProcessed=new Set();let rateLimited=false;let sealedPlanRequired=false;
    ...
    for(let start=0;start<sealedItems.length;start+=5){const batch=sealedItems.slice(start,start+5);const results=await Promise.allSettled(batch.map(async item=>{const id=item.externalIds?.pkmnpricesSealed||String(item.id).replace(/^sealed:/,'');const response=await fetch(`/api/sealed?id=${encodeURIComponent(id)}`,{headers:{Accept:'application/json'}});if(response.status===403){sealedPlanRequired=true;sealedProcessed.add(item.id);return null;}if(response.status===429){rateLimited=true;return null;}if(!response.ok)throw new Error(String(response.status));const payload=await response.json();sealedProcessed.add(item.id);return {item,product:payload.product};}));results.forEach(result=>{if(result.status==='fulfilled'&&result.value?.product)sealedProducts.set(result.value.item.id,result.value.product);});}
    state.watchlist=state.watchlist.map(item=>{const sealed=item.cardState==='sealed';if(sealed&&!sealedProcessed.has(item.id))return {...item,pricingStatus:rateLimited?'rate_limited':'error'};if(!sealed&&!processed.has(item.id))return {...item,pricingStatus:rateLimited?'rate_limited':'error'};const card=sealed?sealedProducts.get(item.id):cards.get(item.id);if(!card)return {...item,currentPrice:null,quotes:[],pricingStatus:sealed&&sealedPlanRequired?'plan_required':'unavailable'};const quote=selectReferenceQuote(card.quotes,item.variant,item.currency||'USD',sealed?{}:item);return {...item,externalIds:{...(item.externalIds||{}),...(card.externalIds||{})},metadata:card.metadata||item.metadata||null,productType:card.productType||item.productType||null,currentPrice:quote?.amount??null,quotes:card.quotes||[],priceHistory:card.history||[],historyStatus:card.historyStatus||null,pricingStatus:quote?quoteStatus(quote):'unavailable',pricingUpdatedAt:quote?.observedAt||quote?.retrievedAt?.slice?.(0,10)||null};});
...
function renderLiquidationPlanner() {
  const output=$('#liquidationOutput');const note=$('#liquidationNote');const exportButton=$('#liquidationExport');if(!output||!note||!exportButton)return;
  const plan=currentLiquidationPlan();
  exportButton.disabled=!plan?.rows.length;
  if(!plan){output.innerHTML='<div class="data-boundary"><strong>Check the scenario inputs</strong><p>Use non-negative percentages and costs. Selling fees must stay below 100%.</p></div>';note.textContent='No estimate is shown until every assumption is valid.';return;}
  if(!plan.rows.length){output.innerHTML='<div class="data-boundary"><strong>No priced USD inventory yet</strong><p>Add an item and load an exact market reference to plan a take-home amount.</p></div>';note.textContent=plan.unpricedUnits?`${plan.unpricedUnits} unpriced card${plan.unpricedUnits===1?' is':'s are'} excluded, not valued at zero.`:'Market references are estimates, not guaranteed sale prices.';return;}
  const profitClass=plan.profitMinor===null?'':plan.profitMinor>=0?'positive':'negative';
  ...
  const omissions=[];if(plan.unpricedUnits)omissions.push(`${plan.unpricedUnits} unpriced item${plan.unpricedUnits===1?'':'s'} excluded`);if(plan.skippedCurrencyUnits)omissions.push(`${plan.skippedCurrencyUnits} non-USD item${plan.skippedCurrencyUnits===1?'':'s'} kept separate`);if(plan.unknownBasisUnits)omissions.push(`${plan.unknownBasisUnits} item${plan.unknownBasisUnits===1?'':'s'} missing cost basis`);note.textContent=omissions.length?`${omissions.join(' · ')}. Missing values are never treated as zero.`:'Every included item has a matching USD reference and recorded cost basis. This is still a planning estimate, not a guaranteed offer.';
}

function downloadLiquidationScenario() {
  const plan=currentLiquidationPlan();if(!plan?.rows.length)return;
  const quote=value=>`"${String(value??'').replaceAll('"','""')}"`;const dollars=minor=>(Number(minor||0)/100).toFixed(2);
  const rows=[['Item','Set','Number','Condition or grade','Quantity','Reference each','Expected sale each','Expected gross','Estimated percentage fee','Cost basis']];
  plan.rows.forEach(row=>rows.push([row.name,row.set,row.number,row.context,row.quantity,dollars(row.referenceUnitMinor),dollars(row.expectedUnitMinor),dollars(row.grossMinor),dollars(row.feesMinor),row.costBasisMinor===null?'':dollars(row.costBasisMinor)]));
  rows.push([],[`Scenario: ${liquidationInputs().referencePercent}% of reference`, `Fee: ${liquidationInputs().feePercent}%`, `Other selling costs: ${dollars(plan.totalSellingCostsMinor)}`, `Estimated take-home: ${dollars(plan.netProceedsMinor)}`, plan.profitMinor===null?'Estimated profit: unavailable — basis incomplete':`Estimated profit: ${dollars(plan.profitMinor)}`]);
  downloadTextFile(rows.map(row=>row.map(quote).join(',')).join('\n'),'text/csv;charset=utf-8',`mica-take-home-scenario-${localIsoDate()}.csv`);toast('Take-home scenario downloaded');
}
...
function renderInventoryHealth() {
  ...
  $('#inventoryHealthNote').textContent=health.unknownBasisQuantity?`${health.unknownBasisQuantity} card${health.unknownBasisQuantity===1?' has':'s have'} unknown acquisition cost, so aging is shown by card count and profit stays unavailable.`:health.skippedCurrencyPositions?`${health.skippedCurrencyPositions} non-USD position${health.skippedCurrencyPositions===1?' was':'s were'} kept separate. Aging uses remaining purchase lots, not the original full purchase.`:'Aging uses remaining FIFO purchase lots. Concentration uses priced positions only; missing prices are not treated as zero.';
}
...
function renderInsights() {
  ...
  const ranked=[...state.items].map(item=>({item,value:item.price==null?null:Number(item.price)*Number(item.quantity),gain:item.price==null||item.costBasis==null?null:Number(item.price)*Number(item.quantity)-Number(item.costBasis)})).sort((a,b)=>(b.value??-1)-(a.value??-1));
  ...
  $('#recentActivity').innerHTML=recent.length?recent.map(({item,transaction})=>`<div class="mover"><img src="${esc(item.thumb)}" alt=""><div><strong>${transaction.type==='purchase'?'Purchased':'Sold'} ${esc(item.name)}</strong><span>${esc(transaction.date||'Date not recorded')} · ${transaction.quantity}${transaction.unitPrice==null?' · cost not recorded':` at ${money(transaction.unitPrice,transaction.currency)}`}</span></div><b>${transaction.type==='purchase'?(transaction.totalCost==null?'Not recorded':money(transaction.totalCost,transaction.currency)):money(transaction.netProceeds,transaction.currency)}</b></div>`).join(''):'<div class="data-boundary"><strong>No transactions yet</strong><p>Purchases and sales will appear here.</p></div>';
...
function renderIntakeQueueBar() {
  const bar=$('#intakeQueueBar');if(!bar)return;bar.classList.toggle('hidden',state.intakeQueue.length===0);$('#intakeQueueCount').textContent=`${state.intakeQueue.length} exact card${state.intakeQueue.length===1?'':'s'} queued`;
}

function queueIntakeCard(card) {
  if(!card)return;state.intakeQueue.push({queueEntryId:crypto.randomUUID(),card});renderIntakeQueueBar();toast(`${card.name} queued · exact printing preserved`);
}

function openBatchIntakeSheet() {
  if(!state.intakeQueue.length){toast('Your intake queue is empty');return;}
  const today=localIsoDate();const entries=[...state.intakeQueue];
  openSheet(`<div class="sheet-heading"><div><h2 id="sheetTitle">Batch add raw cards</h2><p>One condition and purchase date · exact details stay visible for every card.</p></div><button class="sheet-close" aria-label="Close">×</button></div><form id="batchIntakeForm"><div class="form-grid batch-shared-fields"><div class="field"><label for="batchCondition">Condition for every card</label><select id="batchCondition"><option value="near_mint">Near Mint</option><option value="lightly_played">Lightly Played</option><option value="moderately_played">Moderately Played</option><option value="heavily_played">Heavily Played</option><option value="damaged">Damaged</option></select></div><div class="field"><label for="batchDate">Purchase date</label><input id="batchDate" type="date" max="${today}" value="${today}" required></div></div><div class="batch-intake-list">${entries.map((entry,index)=>{const variants=Array.isArray(entry.card.variants)&&entry.card.variants.length?entry.card.variants:[entry.card.variant||'Unknown'];return `<article class="batch-intake-row" data-batch-index="${index}"><img src="${esc(entry.card.thumb||entry.card.image||'./icons/icon.svg')}" alt=""><div class="batch-intake-identity"><strong>${esc(entry.card.name)}</strong><span>${esc(entry.card.set)} · ${esc(entry.card.number)} · ${esc(languageName(entry.card.language||'en'))}</span><label>Exact variant<select data-batch-variant>${variants.map(value=>`<option value="${esc(value)}">${esc(value)}</option>`).join('')}</select></label></div><div class="batch-intake-values"><label>Quantity<input data-batch-quantity type="number" inputmode="numeric" min="1" max="99999" step="1" value="1" required></label><label>Total acquisition cost<div class="money-input"><span>$</span><input data-batch-cost type="number" inputmode="decimal" min="0" step="0.01" placeholder="0.00" required></div></label></div></article>`;}).join('')}</div><p class="form-error" id="batchIntakeStatus" role="status">Nothing is saved until every row is valid.</p><div class="sheet-actions"><button class="secondary" id="batchIntakeBack" type="button">Back to queue</button><button class="primary" type="submit">Add ${entries.length} raw card${entries.length===1?'':'s'}</button></div></form>`);
  $('#batchIntakeBack').addEventListener('click',()=>{closeSheet({discardHistory:true});openIntakeQueueSheet();});
  $('#batchIntakeForm').addEventListener('submit',async event=>{event.preventDefault();const rows=$$('.batch-intake-row',event.currentTarget).map((row,index)=>({id:entries[index].queueEntryId,variant:$('[data-batch-variant]',row).value,quantity:$('[data-batch-quantity]',row).value,totalAcquisitionCost:$('[data-batch-cost]',row).value}));const rawCondition=$('#batchCondition').value;const plan=batchAcquisitionPlan(rows,{rawCondition,transactionDate:$('#batchDate').value},today);if(plan.errors.length){$('#batchIntakeStatus').textContent=plan.errors.slice(0,3).map(error=>error.message).join(' · ');return;}const submit=event.currentTarget.querySelector('[type="submit"]');submit.disabled=true;$('#batchIntakeBack').disabled=true;$('.sheet-close').disabled=true;const saved=new Set();const failures=[];for(const [index,input] of plan.ready.entries()){const entry=entries.find(candidate=>candidate.queueEntryId===input.id);$('#batchIntakeStatus').textContent=`Saving ${index+1} of ${plan.ready.length} securely…`;try{await createPosition(supabase,{...input,identity:identitySnapshot(entry.card,input.variant),cardId:entry.card.cardId||null,variantId:entry.card.variantId||null,grader:null,grade:null,idempotencyKey:crypto.randomUUID(),currency:'USD'});saved.add(input.id);}catch(error){failures.push(`${entry.card.name}: ${error.message||'could not save'}`);}}state.intakeQueue=state.intakeQueue.filter(entry=>!saved.has(entry.queueEntryId));renderIntakeQueueBar();try{await reloadPortfolio();}catch{failures.push('Library refresh will retry automatically');}closeSheet({discardHistory:true});if(failures.length){if(state.intakeQueue.length)openIntakeQueueSheet();toast(`${saved.size} saved · ${failures.length} need review`);}else{routeTo('collection');toast(`${saved.size} raw card${saved.size===1?'':'s'} added to your library`);}});
}
...
function openIntakeQueueSheet() {
  ...
  $('#batchRawIntake').addEventListener('click',()=>{closeSheet({discardHistory:true});openBatchIntakeSheet();});
  $$('[data-intake-prepare]',$('#sheetContent')).forEach(button=>button.addEventListener('click',()=>{const entry=state.intakeQueue.find(item=>item.queueEntryId===button.dataset.intakePrepare);if(!entry)return;closeSheet({discardHistory:true});openPositionSheet(entry.card,{queueEntryId:entry.queueEntryId});}));
  $$('[data-intake-remove]',$('#sheetContent')).forEach(button=>button.addEventListener('click',()=>{state.intakeQueue=state.intakeQueue.filter(item=>item.queueEntryId!==button.dataset.intakeRemove);renderIntakeQueueBar();if(state.intakeQueue.length)openIntakeQueueSheet();else closeSheet({discardHistory:true});}));
  $('#clearIntakeQueue').addEventListener('click',()=>{state.intakeQueue=[];renderIntakeQueueBar();closeSheet({discardHistory:true});toast('Intake queue cleared');});
  $('#intakeDone').addEventListener('click',()=>{closeSheet({discardHistory:true});routeTo('scan');$('#quickCardSearch').focus();});
}
...
function bindQuickCardSearch() {
  ...
  const showResults = results => {
    ...
    $$('[data-queue-card]', resultsNode).forEach(button => button.addEventListener('click', () => queueIntakeCard(catalog.find(card => card.id === button.dataset.queueCard))));
  };
  const search = async () => {
...
function bindEvents() {
  ...
  $('#reviewIntakeQueue').addEventListener('click',openIntakeQueueSheet);renderIntakeQueueBar();
