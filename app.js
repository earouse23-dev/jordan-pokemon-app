...
function openPositionEditSheet(item) {
  const favorite=(item.tags||[]).some(tag=>String(tag).toLowerCase()==='favorites');const labels=(item.tags||[]).filter(tag=>String(tag).toLowerCase()!=='favorites');const today=new Date().toISOString().slice(0,10);const editableStatus=['owned','listed','archived'].includes(item.status)?item.status:'owned';
...
function downloadLiquidationScenario() {
  ...
  downloadTextFile(rows.map(row=>row.map(quote).join(',')).join('\n'),'text/csv;charset=utf-8',`mica-take-home-scenario-${new Date().toISOString().slice(0,10)}.csv`);toast('Take-home scenario downloaded');
}
...
function openBatchIntakeSheet() {
  ...
  const today=new Date().toISOString().slice(0,10);const entries=[...state.intakeQueue];
