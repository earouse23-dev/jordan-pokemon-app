...
function handleCsv(file) {
  ...
  const reader=new FileReader();reader.onerror=()=>toast('Mica could not read that CSV');reader.onload=()=>{
    ...
    const today=localIsoDate();
    openSheet(`<div class="sheet-heading"><div><h2 id="sheetTitle">Import ${records.length.toLocaleString()} card record${records.length===1?'':'s'}?</h2><p>Add this file to your private cloud portfolio.</p></div><button class="sheet-close" aria-label="Close">×</button></div>${errorCopy}<div class="info-copy"><p>Mica preserves exact card state, grade, variant, purchase date, total acquisition cost, certification, labels, location, and notes. Existing positions are never overwritten. Re-importing the same rows with the same fallback date safely reuses positions already saved.</p></div><div class="field"><label for="importFallbackDate">Date for rows missing a purchase date</label><input id="importFallbackDate" type="date" max="${today}" value="${today}" required><small>Rows that already include a purchase date keep their own date.</small></div><section class="import-progress" id="importProgress" aria-labelledby="importProgressTitle" hidden><div><strong id="importProgressTitle">Preparing import…</strong><span id="importProgressCount">0 of ${records.length.toLocaleString()}</span></div><progress id="importProgressBar" max="${records.length}" value="0">0%</progress><small id="importProgressHelp">You can pause after the current secure saves finish.</small></section><p class="form-error" id="importStatus" role="status" aria-live="polite"></p><div class="sheet-actions import-actions"><button class="secondary" id="cancelCsvImport" type="button">Cancel</button><button class="secondary" id="pauseCsvImport" type="button" hidden>Pause</button><button class="primary" id="addCsvImport" type="button">Add to my account</button></div>`);
