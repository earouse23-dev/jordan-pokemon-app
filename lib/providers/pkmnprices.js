...
export function normalizePkmnPricesCard(card, historyRows = [], retrievedAt = new Date().toISOString(), clientId = null, historyStatus = 'live') {
  ...
  for (const point of Array.isArray(historyRows) ? historyRows : []) {
    ...
    history.push({
      provider,
      providerVariantId: [card.id, provider, point.condition || '', point.variant || '', point.grader || '', point.grade || ''].join(':'),
...
async function request(url, apiKey, signal) {
  ...
  for(let attempt=0;attempt<3;attempt+=1){
    try{
      const response=await fetch(url,{headers:{'X-API-Key':apiKey,Accept:'application/json'},signal});const body=await response.json().catch(()=>({}));
      if(response.ok)return body;
      const error=new Error('PkmnPrices request failed');error.status=response.status;error.providerCode=String(body?.error?.code||'');error.providerMessage=String(body?.error?.message||'');error.retryable=response.status===429||response.status>=500;error.retryAfter=response.headers.get('retry-after');
      if(!error.retryable||attempt===2)throw error;lastError=error;
    }catch(error){lastError=error;if(signal?.aborted||(!error?.retryable&&error?.status))throw error;if(attempt===2)throw error;}
