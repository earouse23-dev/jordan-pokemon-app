...
function safeCardmarketUrl(value) {
  try {
    const url = new URL(value);
    return url.protocol === "https:" && /(^|\.)cardmarket\.com$/i.test(url.hostname)
      ? url.toString()
      : null;
  } catch {
    return null;
...
export function normalizePkmnPricesCard(
  ...
  for (const price of Array.isArray(card?.prices) ? card.prices : []) {
    ...
    const baseQuote = {
      ...
      retrievedAt,
      providerUrl:
        provider === "cardmarket" ? safeCardmarketUrl(card.cardmarket_url) : null,
...
export function normalizePkmnPricesOffer(listing, marketplace) {
  ...
  return {
    ...
    sellerRating:
      Number.isFinite(Number(listing.seller_rating))
...
export function normalizePkmnPricesSealedProduct(
  ...
  return {
    ...
    images: { small: product.image_url || null, large: product.image_url || null },
    quotes,
  };
}
...
async function resolveCardId(apiKey, lookup, signal) {
  if (lookup.pkmnpricesId) return lookup.pkmnpricesId;
  ...
  return (
    selectCard(Array.isArray(result.data) ? result.data : [], lookup)?.id || null
  );
}
...
export async function fetchPkmnPricesSealedSearch(
  ...
  url.searchParams.set(
    "language",
    String(language || "en").toLowerCase().startsWith("ja") ? "jp" : "en",
  );
