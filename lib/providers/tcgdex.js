...
function rankTcgdexCard(card, parsed, language) {
  ...
  if (wantedName) {
    ...
    if (cardName === wantedName) { score += 130; reasons.push('exact name'); nameMatchQuality = 'exact'; }
    else if (cardName.startsWith(wantedName)) { score += 90; reasons.push('name starts with search'); nameMatchQuality = 'prefix'; }
    else {
      const tokens = wantedName.split(' ').filter(Boolean);
      const matched = tokens.filter(token => cardName.includes(token)).length;
      score += matched * 22;
      if (matched === tokens.length) { reasons.push('all name terms'); nameMatchQuality = 'tokens'; }
    }
  }
  ...
  const numberOnlyAlternative = wantedName && !nameSupportsConfidence && reasons.some(reason => reason.includes('number'));
  return {
    score,
    reasons,
