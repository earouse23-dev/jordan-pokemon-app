...
function collectorPart(value) {
  ...
  return match
    ? `${match[1].toUpperCase()}${Number(match[2])}${match[3].toUpperCase()}`
...
export function parseCatalogQuery(query) {
  ...
  const hints = [];
  const providerId =
    /^[A-Za-z0-9.]+-[A-Za-z0-9]+$/.test(original) && !original.includes("/")
      ? original
      : null;
  if (providerId)
    return {
  ...
  const numberMatch =
    /\b([A-Za-z]{0,3}\d{1,4}[A-Za-z]?)\s*\/\s*([A-Za-z]{0,3}\d{1,4}[A-Za-z]?)\b/i.exec(
      remainder,
    );
  const explicitNumber =
    /(?:#|\bno\.?\s*)([A-Za-z]{0,3}\d{1,4}[A-Za-z]?)\b/i.exec(remainder);
  ...
  if (numberMatch) remainder = remainder.replace(numberMatch[0], " ");
    ...
    remainder = remainder.replace(explicitNumber[0], " ");
  ...
  let setCode = "";
  const setCodeMatch = /\b(?:set\s+)?([a-z]{1,8}\d{1,3}(?:\.\d+)?)\b/i.exec(
    remainder,
  );
  if (setCodeMatch) {
...
export function rankTcgdexCard(card, parsed, language) {
  let score = 0;
  const reasons = [];
  ...
  let nameMatchQuality = wantedName ? "none" : "not-requested";
  if (wantedName) {
    ...
    if (cardName === wantedName) {
    ...
    } else if (cardName.startsWith(wantedName)) {
      score += 90;
      reasons.push("name starts with search");
      nameMatchQuality = "prefix";
    } else {
      ...
      if (matched === tokens.length) {
        ...
        nameMatchQuality = "tokens";
      }
    }
  }
  if (parsed.localId)
    exactChecks.push(cardLocal === collectorPart(parsed.localId));
  if (parsed.localId && cardLocal === collectorPart(parsed.localId)) {
    score += 125;
    reasons.push("exact collector number");
  }
  if (parsed.total)
    exactChecks.push(officialTotal === collectorPart(parsed.total));
  if (parsed.total && officialTotal === collectorPart(parsed.total)) {
    score += 105;
    reasons.push("exact set number");
  }
  if (wantedSet) exactChecks.push(cardSet === wantedSet);
  if (wantedSet && cardSet === wantedSet) {
    score += 115;
    reasons.push("exact set");
  } else if (wantedSet && cardSet.includes(wantedSet)) {
    score += 70;
    reasons.push("matching set");
  }
  if (parsed.setCode)
    exactChecks.push(comparable(card.set?.id) === comparable(parsed.setCode));
  if (
    parsed.setCode &&
    comparable(card.set?.id) === comparable(parsed.setCode)
  ) {
    score += 115;
    reasons.push("exact set code");
  }
  if (parsed.providerId) exactChecks.push(card.id === parsed.providerId);
  if (parsed.providerId && card.id === parsed.providerId) {
    score += 400;
    reasons.push("exact provider ID");
  }
      ...
      hintWords
        ...
        .every(
          (token) =>
            ...
            (token === "first" && hintText.includes("firstedition")),
        )
    ) {
      score += 18;
  ...
  if (language) score += 2;
  const allExplicitDetailsMatch =
    exactChecks.length > 0 && exactChecks.every(Boolean);
  const nameSupportsConfidence = nameMatchQuality !== "none";
  const numberOnlyAlternative =
    wantedName &&
    !nameSupportsConfidence &&
    reasons.some((reason) => reason.includes("number"));
  return {
    score,
    reasons,
    confidence: allExplicitDetailsMatch
      ? "Exact match"
      : score >= 130 && nameSupportsConfidence
        ? "Strong match"
        : numberOnlyAlternative
          ? "Number-only alternative"
          : "Possible match",
  };
}
...
function finishFromPricingKey(value) {
  ...
  if (key.includes("1st") && (key.includes("holo") || key.includes("foil")))
  ...
  if (
    ...
    (key.includes("holo") || key.includes("foil"))
  )
    return "holofoil";
...
export function normalizeTcgdexCard(card, language) {
  ...
  return {
    ...
    set: card.set?.name || "",
    setId: card.set?.id || "",
    number: fullCollectorNumber(card),
    localId: card.localId || "",
    rarity: card.rarity || null,
    artist: card.illustrator || null,
    language,
    release: card.set?.releaseDate || null,
    variants,
    image: images.large,
    imageProvider: "tcgdex",
    referenceImages: [
      ...(images.large
        ? [{ provider: "tcgdex", size: "large", url: images.large }]
        : []),
      ...(images.small
        ? [{ provider: "tcgdex", size: "small", url: images.small }]
        : []),
    ],
    thumb: images.small,
  };
...
export async function fetchTcgdexSet(setId, language = "en", signal) {
  const set = await fetchJson(
    `${API_URL}/${language}/sets/${encodeURIComponent(setId)}`,
    signal,
  );
...
export function normalizeTcgdexPricingCard(
  card,
    ...
    for (const [variantName, prices] of Object.entries(tcgplayer)) {
      ...
      for (const [field, priceType] of Object.entries(PRICE_FIELDS)) {
        ...
        quotes.push({
          ...
          providerVariantId: `${card.id}:${variantName}`,
          ...
          priceType,
          amount: value,
          observedAt: tcgplayer.updated || null,
          retrievedAt,
          providerUrl: null,
          attribution: "TCGplayer market pricing via TCGdex",
          derivation: "aggregated",
          quality: {
            direct: false,
            aggregator: "tcgdex",
            field,
            sourceFrequency: "hourly-to-daily",
          },
        });
      }
    }
  ...
  if (cardmarket && typeof cardmarket === "object") {
    for (const [field, rawValue] of Object.entries(cardmarket)) {
      ...
      quotes.push({
        ...
        market: "cardmarket",
        providerProductId: String(card.id),
        providerVariantId: `${card.id}:${holo ? "holo" : "normal"}`,
        currency: cardmarket.unit || "EUR",
        region: "EU",
        condition: null,
        finish: holo ? "holofoil" : "normal",
        printing: holo ? "holo" : "normal",
        language,
        gradingCompany: null,
        grade: null,
        priceType,
        amount: value,
        observedAt: cardmarket.updated || null,
        retrievedAt,
        providerUrl: null,
        attribution: "Cardmarket pricing via TCGdex",
        derivation: "aggregated",
        quality: {
          direct: false,
          aggregator: "tcgdex",
  ...
  return {
    providerCardId: clientId || `tcgdex:${language}:${card.id}`,
    ...
    capabilities: {
      ...
      raw: quotes.length ? "live" : "missing",
      graded: "unsupported",
      history: "unsupported",
      eur: quotes.some((quote) => quote.currency === "EUR")
        ? "live"
        : "missing",
      completedSales: "unsupported",
      askingPrices: "unsupported",
      sealed: "unsupported",
    },
  };
}
...
function externalIdFromLookup(lookup, language) {
  ...
  if (String(lookup.clientId).startsWith(prefix))
    return String(lookup.clientId).slice(prefix.length);
  if (/^[A-Za-z0-9.-]+-[A-Za-z0-9.-]+$/.test(lookup.clientId))
    return lookup.clientId;
  return null;
}

export async function fetchTcgdexPricingLookup(
  lookup,
  signal,
  language = "en",
) {
  ...
  if (externalId) {
    const direct = await fetchJson(
      `${API_URL}/${language}/cards/${encodeURIComponent(externalId)}`,
      signal,
    );
  ...
  const details = await Promise.all(
    (Array.isArray(briefs) ? briefs : [])
      ...
      .map((brief) =>
        fetchJson(
          `${API_URL}/${language}/cards/${encodeURIComponent(brief.id)}`,
          signal,
        ),
      ),
  );
...
export async function searchTcgdexCards(query, language, limit, signal) {
  ...
  if (parsed.providerId) {
    const card = await fetchJson(
      `${API_URL}/${language}/cards/${encodeURIComponent(parsed.providerId)}`,
      signal,
    );
    if (!card?.id) return [];
  ...
  const addShape = (shape) => {
    ...
    if (!queryShapes.some((candidate) => candidate.key === key))
      queryShapes.push({ key, entries });
  };
  const exact = {
    name: parsed.name,
    localId: parsed.localId,
    "set.name": parsed.setName,
    "set.id": parsed.setCode,
    "set.cardCount.official": parsed.total,
  };
  addShape(exact);
  ...
  const pages = await Promise.all(
    searches.map((url) => fetchJson(url, signal)),
  );
  if (!pages.some(Array.isArray)) throw new Error("TCGdex search failed");
  const briefs = new Map();
  for (const page of pages)
    for (const brief of Array.isArray(page) ? page : [])
      if (brief?.id && !briefs.has(brief.id)) briefs.set(brief.id, brief);
  ...
  return details
    .filter(Boolean)
    .filter((card) => {
      if (parsed.name && words(card.name) !== words(parsed.name)) return false;
      if (
        parsed.localId &&
        collectorPart(card.localId) !== collectorPart(parsed.localId)
      )
        return false;
      if (
        parsed.total &&
        collectorPart(card.set?.cardCount?.official) !==
          collectorPart(parsed.total)
      )
        return false;
      if (parsed.setName && words(card.set?.name) !== words(parsed.setName))
        return false;
      if (
        parsed.setCode &&
        comparable(card.set?.id) !== comparable(parsed.setCode)
      ...
      if (parsed.hints.length) {
        const available = [
          ...Object.entries(card.variants || {})
            .filter(([, enabled]) => enabled)
            .map(([variant]) => words(variant)),
          words(card.rarity),
        ].join(" ");
        if (
          parsed.hints.some(
            (hint) => !available.includes(words(hint).split(" ")[0]),
          )
        )
          return false;
      }
