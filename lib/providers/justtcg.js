...
export function normalizePrinting(value) {
  const printing = String(value || "")
    .trim()
    .toLowerCase();
  if (printing.includes("1st") && printing.includes("holo"))
...
export function normalizeJustTcgCard(
  ...
  for (const variant of Array.isArray(card?.variants) ? card.variants : []) {
    ...
    for (const point of Array.isArray(variant.priceHistory)
  ...
  return {
    ...
    externalIds: {
      justtcg: card.uuid || card.id || null,
      tcgplayer: card.tcgplayerId || null,
    },
    name: card.name || "",
    ...
    capabilities: {
      ...
      graded: "unsupported",
      history: history.length ? "live" : "missing",
      eur: "unsupported",
...
export async function fetchJustTcgLookup(apiKey, lookup, signal) {
  ...
  else if (lookup.tcgplayerId)
    url.searchParams.set("tcgplayerId", lookup.tcgplayerId);
  else {
  ...
  return {
    card:
      lookup.justtcgId || lookup.tcgplayerId
        ? cards[0] || null
        : selectMatch(cards, lookup),
