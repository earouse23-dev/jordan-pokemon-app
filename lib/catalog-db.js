...
function internalCard(row, language) {
  ...
  const variantOptions = (
    ...
    .filter(
      (entry) =>
        !entry?.language || String(entry.language).toLowerCase() === language,
    )
    .map((entry) =>
      normalizeVariantOption(
        {
          ...
          promoType:
            entry.metadata?.promoType ||
            entry.metadata?.promo_type ||
            (entry.is_promo ? "promo" : "none"),
        },
        { language },
      ),
    );
  const finishes = [...new Set(variantOptions.map((entry) => entry.label))];
  ...
  return {
    ...
    cardId: row.id,
    collectibleId:
      variantOptions.length === 1 ? variantOptions[0].collectibleId : null,
    name: row.name || "",
    set: set.name || "",
    setId: setTcgdexId || "",
...
export function summarizeCatalogResolution(cards, parsed) {
  ...
  const variantResolution = resolveIdentityCandidates(
    {
      name: parsed.name,
      set: parsed.setName,
      number: parsed.localId,
      language: top.language,
      finish: parsed.hints?.find((hint) => /holo|parallel|normal/i.test(hint)),
      edition: parsed.hints?.find((hint) =>
        /edition|shadowless|parallel/i.test(hint),
      ),
    },
    (top.variantOptions || []).map((option) => ({
      ...option,
      name: top.name,
      set: top.set,
      number: top.localId || top.number,
    })),
  );
  const ambiguity = [];
  ...
  if (recommended && (top.variantOptions || []).length > 1)
    ambiguity.push("multiple_variants");
  return {
    status: recommended ? "exact" : "review",
    recommendedId: recommended ? top.id : null,
    recommendedCollectibleId:
      recommended && variantResolution.status === "exact"
        ? variantResolution.recommendedId
        : null,
