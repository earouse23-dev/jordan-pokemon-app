...
export function batchAcquisitionPlan(
  ...
  for (const [index, row] of (rows || []).entries()) {
    ...
    if (!String(row.variant || "").trim())
      errors.push({ id: row.id, message: `Card ${index + 1}: choose an exact variant.` });
    else if (!breakdown)
      errors.push({ id: row.id, message: `Card ${index + 1}: enter a valid quantity and total cost.` });
    else if (!validation.valid)
      errors.push({ id: row.id, message: `Card ${index + 1}: ${Object.values(validation.errors)[0]}` });
...
export function listingReadiness(items = [], today = new Date().toISOString().slice(0, 10)) {
...
export function liquidationPlan(
  ...
  result.netProceedsMinor =
    result.expectedGrossMinor -
    result.marketplaceFeesMinor -
    sellingCostsMinor;
