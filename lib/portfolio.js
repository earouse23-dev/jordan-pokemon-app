...
export function batchAcquisitionPlan(
  rows,
  { rawCondition, transactionDate },
  ...
  for (const [index, row] of (rows || []).entries()) {
    ...
    const breakdown = acquisitionFromTotal(row.totalAcquisitionCost, quantity);
    const input = {
      ...breakdown,
      cardState: "raw",
      rawCondition,
    ...
    const validation = breakdown ? validateAcquisition(input, today) : null;
    if (!String(row.variant || "").trim())
      errors.push({
        id: row.id,
        message: `Card ${index + 1}: choose an exact variant.`,
      });
    else if (!breakdown)
      errors.push({
        id: row.id,
        message: `Card ${index + 1}: enter a valid quantity and total cost.`,
      });
    else if (!validation.valid)
      errors.push({
        id: row.id,
        message: `Card ${index + 1}: ${Object.values(validation.errors)[0]}`,
      });
    else ready.push({ ...row, ...input });
  }
  return { ready, errors };
}
...
export function blendedPosition({
  ...
  const owned = Number(currentQuantity);
  const added = Number(newQuantity);
  const basis = toMinorUnits(currentCostBasis);
  const addedCost = toMinorUnits(newTotalCost);
  const price =
    currentUnitPrice === null || currentUnitPrice === undefined
      ? null
      : toMinorUnits(currentUnitPrice);
  if (
    ...
    return null;
  const quantity = owned + added;
  const costBasisMinor = basis + addedCost;
  const averageCostMinor = Math.round(costBasisMinor / quantity);
  const currentAverageCostMinor = Math.round(basis / owned);
  const marketValueMinor = price === null ? null : price * quantity;
  return {
    quantity,
    costBasisMinor,
    averageCostMinor,
    currentAverageCostMinor,
    averageChangeMinor: averageCostMinor - currentAverageCostMinor,
    marketValueMinor,
    unrealizedGainMinor:
      marketValueMinor === null ? null : marketValueMinor - costBasisMinor,
  };
}

export function purchaseEntryPoints(
  transactions = [],
  currentUnitPrice = null,
) {
  const currentMinor =
  ...
  return transactions
    ...
    .map((transaction) => {
      ...
      const explicitTotal = toMinorUnits(transaction.totalCost);
      const unit = toMinorUnits(transaction.unitPrice);
      ...
      const totalCostMinor =
        explicitTotal === null
          ? unit === null
            ...
            : unit * quantity
          : explicitTotal;
      if (totalCostMinor === null || totalCostMinor < 0) return null;
      const unitCostMinor = Math.round(totalCostMinor / quantity);
      const marketAtPurchaseMinor = toMinorUnits(
        transaction.marketUnitPriceAtPurchase,
      );
      const changeMinor =
        currentMinor === null ? null : currentMinor - unitCostMinor;
      return {
        date: String(transaction.date || ""),
        quantity,
        totalCostMinor,
        unitCostMinor,
        marketAtPurchaseMinor,
        marketPriceProvider: transaction.marketPriceProvider || "",
        marketPriceObservedAt: transaction.marketPriceObservedAt || null,
        currentUnitPriceMinor: currentMinor,
        changeMinor,
        returnPercent:
          changeMinor === null || unitCostMinor === 0
            ? null
            : (changeMinor / unitCostMinor) * 100,
      };
    })
    .filter(Boolean)
...
export function buyOfferPlan({
  quantity = 1,
  expectedResaleEach,
  feePercent = 0,
  otherSellingCosts = 0,
  targetRoiPercent = 0,
  plannedOfferEach = null,
}) {
  const count = Number(quantity);
  const resaleEachMinor = toMinorUnits(expectedResaleEach);
  const otherCostsMinor = toMinorUnits(otherSellingCosts);
  const feeRate = Number(feePercent);
  const targetRate = Number(targetRoiPercent);
  const offerEachMinor =
    plannedOfferEach === null || String(plannedOfferEach).trim() === ""
      ? null
      : toMinorUnits(plannedOfferEach);
  if (
    !Number.isInteger(count) ||
    count <= 0 ||
    resaleEachMinor === null ||
    resaleEachMinor < 0 ||
    ...
    targetRate < 0 ||
    (offerEachMinor !== null && offerEachMinor < 0)
  )
    return null;

  const grossMinor = resaleEachMinor * count;
  const marketplaceFeesMinor = Math.round((grossMinor * feeRate) / 100);
  const netBeforeAcquisitionMinor =
    grossMinor - marketplaceFeesMinor - otherCostsMinor;
  const maxAcquisitionMinor = Math.max(
    0,
    Math.floor(netBeforeAcquisitionMinor / (1 + targetRate / 100)),
  );
  const maxOfferEachMinor = Math.floor(maxAcquisitionMinor / count);
  const plannedOfferTotalMinor =
    offerEachMinor === null ? null : offerEachMinor * count;
  const projectedProfitMinor =
    plannedOfferTotalMinor === null
      ? null
      : netBeforeAcquisitionMinor - plannedOfferTotalMinor;

  return {
    grossMinor,
    marketplaceFeesMinor,
    otherSellingCostsMinor: otherCostsMinor,
    netBeforeAcquisitionMinor,
    maxAcquisitionMinor,
    maxOfferEachMinor,
    plannedOfferTotalMinor,
    projectedProfitMinor,
    projectedRoiPercent:
      plannedOfferTotalMinor === null || plannedOfferTotalMinor === 0
        ? null
        : (projectedProfitMinor / plannedOfferTotalMinor) * 100,
  };
}
...
export function listingReadiness(
  ...
  const reviewItems = listingReviewItems(listed, today);
  const result = {
    positions: listed.length,
    units: 0,
    askingValueMinor: 0,
    marketValueMinor: 0,
    pricedPositions: 0,
  ...
  for (const item of listed) {
    ...
    const askMinor = toMinorUnits(item.askingPrice);
    const marketMinor = toMinorUnits(item.price);
    result.units += quantity;
    if (askMinor === null) result.missingAsk += 1;
    else result.askingValueMinor += askMinor * quantity;
    if (marketMinor !== null) {
      result.marketValueMinor += marketMinor * quantity;
      result.pricedPositions += 1;
    }
    if (!String(item.listingVenue || "").trim()) result.missingVenue += 1;
  }
  return result;
}

export function listingReviewItems(
  items = [],
  today = new Date().toISOString().slice(0, 10),
) {
  ...
  return (items || [])
    ...
    .map((item) => {
      ...
      const askMinor = toMinorUnits(item.askingPrice);
      const marketMinor = toMinorUnits(item.price);
      if (askMinor === null) reasons.push("asking price missing");
      if (!String(item.listingVenue || "").trim())
        reasons.push("venue missing");
      const reviewedMs = item.priceReviewedAt
        ? Date.parse(`${item.priceReviewedAt}T00:00:00Z`)
        : NaN;
      if (
        !Number.isFinite(reviewedMs) ||
        !Number.isFinite(todayMs) ||
        (todayMs - reviewedMs) / 86400000 > 7
      )
        reasons.push("price review older than 7 days");
      if (
        askMinor !== null &&
        marketMinor !== null &&
        marketMinor > 0 &&
        Math.abs(askMinor - marketMinor) / marketMinor >= 0.1
      )
        reasons.push("ask differs from market by 10% or more");
      return reasons.length ? { ...item, listingReviewReasons: reasons } : null;
    })
    .filter(Boolean);
}
...
export function liquidationPlan(
  ...
  const referenceRate = Number(referencePercent);
  const feeRate = Number(feePercent);
  const sellingCostsMinor = toMinorUnits(totalSellingCosts);
  if (
  ...
  const result = {
    ...
    pricedUnits: 0,
    unpricedUnits: 0,
    skippedCurrencyUnits: 0,
    unknownBasisUnits: 0,
    referenceValueMinor: 0,
    expectedGrossMinor: 0,
    marketplaceFeesMinor: 0,
    totalSellingCostsMinor: sellingCostsMinor,
    netProceedsMinor: 0,
    knownCostBasisMinor: 0,
    profitMinor: null,
    roiPercent: null,
    breakEvenReferencePercent: null,
    rows: [],
  };
  ...
  for (const position of positions || []) {
    ...
    if ((position.currency || "USD") !== currency) {
      result.skippedCurrencyUnits += quantity;
      continue;
    }
    const unitPrice = toMinorUnits(position.price);
    if (unitPrice === null || unitPrice < 0) {
      result.unpricedUnits += quantity;
      continue;
    }
    const expectedUnitMinor = Math.round((unitPrice * referenceRate) / 100);
    const grossMinor = expectedUnitMinor * quantity;
    const feesMinor = Math.round((grossMinor * feeRate) / 100);
    const basisMinor = toMinorUnits(position.costBasis);
    if (basisMinor === null || basisMinor < 0)
      result.unknownBasisUnits += quantity;
    else result.knownCostBasisMinor += basisMinor;
    result.positionCount += 1;
    result.pricedUnits += quantity;
    result.referenceValueMinor += unitPrice * quantity;
    result.expectedGrossMinor += grossMinor;
    result.marketplaceFeesMinor += feesMinor;
    result.rows.push({
      name: String(position.name || "Unknown item"),
      set: String(position.set || ""),
      number: String(position.number || ""),
      context: String(
        position.gradingCompany
          ? `${position.gradingCompany} ${position.grade || ""}`.trim()
          : position.condition || position.variant || "",
      ),
      quantity,
      referenceUnitMinor: unitPrice,
      expectedUnitMinor,
      grossMinor,
      feesMinor,
  ...
  result.netProceedsMinor =
    result.expectedGrossMinor - result.marketplaceFeesMinor - sellingCostsMinor;
  if (!result.unknownBasisUnits) {
    result.profitMinor = result.netProceedsMinor - result.knownCostBasisMinor;
    result.roiPercent =
      result.knownCostBasisMinor === 0
        ? null
        : (result.profitMinor / result.knownCostBasisMinor) * 100;
    const retainedRate = 1 - feeRate / 100;
    if (result.referenceValueMinor > 0 && retainedRate > 0) {
      result.breakEvenReferencePercent =
        ((result.knownCostBasisMinor + sellingCostsMinor) /
          (result.referenceValueMinor * retainedRate)) *
        100;
    }
  }
  return result;
...
export function portfolioReview(
  ...
  const belowCost = positions.filter(
    (item) =>
      ...
      item.costBasis !== null &&
      item.costBasis !== undefined &&
      Number(item.price) * Number(item.quantity || 0) < Number(item.costBasis),
  );
...
export function portfolioActions(
  ...
  const review = portfolioReview(positions, watchlist, today);
  const listingReview = listingReviewItems(positions, today);
  return [
    {
      key: "targets",
      priority: 1,
      title: "Buy targets reached",
      copy: "Check the exact match before the market moves.",
      items: review.reachedTargets,
    },
    {
      key: "listings",
      priority: 2,
      title: "Review active listings",
      copy: "Repair stale prices or missing sale details before buyers see them.",
      items: listingReview,
    },
    {
      key: "pricing",
      priority: 3,
      title: "Fix price gaps",
      copy: "Confirm identity or refresh pricing so totals stay honest.",
      items: review.needsPricing,
    },
    {
      key: "below-cost",
      priority: 4,
      title: "Review positions below cost",
      copy: "Use the sale planner before repricing, holding, or exiting.",
      items: review.belowCost,
    },
    {
      key: "older",
      priority: 5,
      title: "Review older inventory",
      copy: "Decide whether to hold, grade, reprice, or sell.",
      items: review.olderInventory,
    },
...
export function allocateFifo(lots, saleQuantity) {
  ...
  for (const lot of ordered) {
    ...
    const lotCostKnown =
      ...
      lot.totalCostMinor !== null &&
      lot.totalCostMinor !== undefined &&
      Number.isFinite(Number(lot.totalCostMinor));
    const lotCost = lotCostKnown ? Number(lot.totalCostMinor) : 0;
    const allocated = lotCostKnown
      ? quantity === available
    ...
    if (allocated === null) allocatedCostKnown = false;
    else allocatedCost += allocated;
    remaining -= quantity;
  }
  return {
    allocations,
    allocatedQuantity: Number(saleQuantity) - remaining,
    allocatedCost: allocatedCostKnown ? allocatedCost : null,
...
export function positionPerformance(input) {
  ...
  const cost =
    input.remainingCostBasisMinor === null ||
    input.remainingCostBasisMinor === undefined
      ? null
      : Number(input.remainingCostBasisMinor);
  const price =
  ...
  const unrealized =
    currentValue === null || cost === null ? null : currentValue - cost;
  const realized =
    input.netSaleProceedsMinor === null ||
    input.netSaleProceedsMinor === undefined ||
    input.allocatedSoldCostMinor === null ||
    input.allocatedSoldCostMinor === undefined
      ? null
      : Number(input.netSaleProceedsMinor) -
        Number(input.allocatedSoldCostMinor);
  return {
  ...
  for (const position of included) {
    ...
    const agingRows = lots.length
      ? lots.map((lot) => ({
          ...
          costBasis:
            lot.costBasisKnown === false || lot.remainingCost == null
              ? null
              : Number(lot.remainingCost),
        }))
      : [
          {
            ...
            costBasis:
              position.costBasis == null ? null : Number(position.costBasis),
          },
        ];
    for (const row of agingRows) {
      const bucket = bucketFor(row.date);
      bucket.quantity += row.quantity;
      if (row.costBasis === null) unknownBasisQuantity += row.quantity;
      else bucket.costBasis += row.costBasis;
...
export function marketAdjustedPortfolioHistory(
  snapshots,
  transactions = [],
  currency = "USD",
) {
  ...
  for (const snapshot of snapshots || []) {
    const date = String(snapshot.date || snapshot.snapshotDate || "").slice(
      0,
      10,
    );
    const totalMinor = toMinorUnits(snapshot.total);
    if (
      !/^\d{4}-\d{2}-\d{2}$/.test(date) ||
      totalMinor === null ||
      totalMinor < 0 ||
      String(snapshot.currency || normalizedCurrency).toUpperCase() !==
        normalizedCurrency
    )
      continue;
    byDate.set(date, {
      date,
      totalMinor,
      pricedItems: Math.max(0, Number(snapshot.pricedItems) || 0),
      unpricedItems: Math.max(0, Number(snapshot.unpricedItems) || 0),
      freshItems: Math.max(0, Number(snapshot.freshItems) || 0),
    });
  }
  ...
  if (!points.length)
    return { currency: normalizedCurrency, points: [], status: "empty" };
  ...
  const undatedCashFlow = (transactions || []).some(
    (transaction) =>
      cashFlowTypes.includes(transaction.type) &&
      String(transaction.currency || normalizedCurrency).toUpperCase() ===
        normalizedCurrency &&
    ...
    .map((transaction) => {
      ...
      const amount =
        type === "sale"
          ...
          : ["purchase", "grading_return", "fee"].includes(type)
  ...
  const historyPoints = points.map((point) => {
    const periodFlows = flows.filter(
      (flow) => flow.date > baseline.date && flow.date <= point.date,
    );
    const unknownFlow =
      undatedCashFlow ||
      periodFlows.some(
        (flow) =>
          !flow.understood ||
          (["purchase", "sale", "grading_return", "fee"].includes(flow.type) &&
            flow.amountMinor === null),
      );
    const completeCoverage =
      baseline.unpricedItems === 0 &&
      point.unpricedItems === 0 &&
      baseline.freshItems >= baseline.pricedItems &&
      point.freshItems >= point.pricedItems;
    const netContributionMinor = periodFlows.reduce(
      (sum, flow) => sum + (flow.contributionMinor || 0),
      0,
    );
    const marketChangeMinor =
      completeCoverage && !unknownFlow
        ? point.totalMinor - baseline.totalMinor - netContributionMinor
        : null;
    const startTime = new Date(`${baseline.date}T00:00:00Z`).getTime();
    const endTime = new Date(`${point.date}T00:00:00Z`).getTime();
    const duration = endTime - startTime;
    const weightedFlows = periodFlows.reduce((sum, flow) => {
      if (flow.contributionMinor === null || duration <= 0) return sum;
      const flowTime = new Date(`${flow.date}T00:00:00Z`).getTime();
      const weight = Math.max(0, Math.min(1, (endTime - flowTime) / duration));
      return sum + flow.contributionMinor * weight;
    }, 0);
    const denominator = baseline.totalMinor + weightedFlows;
    return {
      ...point,
      netContributionMinor,
      marketChangeMinor,
      returnPercent:
        marketChangeMinor === null || denominator <= 0
          ? null
          : (marketChangeMinor / denominator) * 100,
      completeCoverage,
      unknownFlow,
    };
  });
...
export function rebasePortfolioSnapshots(snapshots, currentTotal) {
  const rows = Array.isArray(snapshots) ? snapshots : [];
  const parsedTotal = Number(currentTotal);
  const targetMinor = Number.isFinite(parsedTotal)
    ? Math.round(parsedTotal * 100)
    : null;
  if (!rows.length || targetMinor === null || targetMinor < 0) return rows;

  let lastIndex = -1;
  let lastMinor = null;
  let latestDate = "";
  for (let index = 0; index < rows.length; index += 1) {
    const value = toMinorUnits(rows[index]?.total);
    const date = String(
      rows[index]?.date || rows[index]?.snapshotDate || "",
    ).slice(0, 10);
    if (
      value !== null &&
      value > 0 &&
      (/^\d{4}-\d{2}-\d{2}$/.test(date) ? date >= latestDate : lastIndex < 0)
    ) {
      lastIndex = index;
      lastMinor = value;
  ...
  if (lastIndex < 0 || lastMinor === null) return rows;

  const ratio = targetMinor / lastMinor;
  return rows.map((row, index) => {
    ...
    return { ...row, total: totalMinor / 100 };
  });
}
