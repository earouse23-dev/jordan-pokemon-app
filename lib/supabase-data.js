...
export function hydratePosition(
  ...
  const manualPrice =
    row.valuation_basis === "manual" ? number(row.manual_value) : null;
  const savedPrice = manualPrice ?? number(identity.showcasePrice);
  return {
    ...
    manualPrice,
    demoPrice: number(identity.showcasePrice),
    price: savedPrice,
    move: null,
    pricingStatus:
      savedPrice === null
...
export async function loadPortfolio(client, ownerId = "") {
  ...
  const [transactions, lots, submissions, historyResult] = await Promise.all([
