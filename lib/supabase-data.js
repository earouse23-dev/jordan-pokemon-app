...
export async function loadPortfolio(client) {
  const { data: rows, error } = await client
    ...
    .select(
      "id,user_id,card_id,variant_id,identity_snapshot,card_state,raw_condition,grader,grade,certification_number,quantity,notes,storage_location,status,currency,created_at,updated_at",
    )
    .order("created_at", { ascending: false });
  if (error) throw error;
  if (!rows?.length) return [];
  const ids = rows.map((row) => row.id);
  const [transactionResult, lotResult] = await Promise.all([
    client
...
export async function updatePosition(client, id, values) {
  const { error } = await client
    ...
    .update({
      ...
      certification_number: values.certificationNumber,
      status: values.status,
      image_override_url: values.imageOverrideUrl,
    })
    .eq("id", id);
  if (error) throw error;
}
