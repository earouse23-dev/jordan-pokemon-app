...
export async function signUpWithPassword(client, email, password) {
  const returnTo = accountReturnUrl();
...
export async function sendPasswordReset(client, email) {
  const returnTo = accountReturnUrl("?auth=reset");
        ...
        const transactionAllocatedCost = saleAllocations.some(
          ...
          : saleAllocations.length
            ? saleAllocations.reduce(
                (sum, allocation) =>
                  sum + Number(allocation.allocated_cost || 0),
                0,
              )
            : null;
...
export async function loadPortfolio(client, ownerId = "") {
  const rows = await loadRowsInPages(client, {
    table: "collection_items",
    columns:
      "id,user_id,collectible_id,card_id,variant_id,identity_snapshot,card_state,raw_condition,grader,grade,certification_number,quantity,valuation_basis,manual_value,notes,storage_location,tags,status,asking_price,listing_venue,listed_at,price_reviewed_at,currency,created_at,updated_at",
    order: "created_at",
    secondaryOrder: "id",
    ascending: false,
  ...
  const [
    ...
    loadRowsInChunks(client, {
      ...
      ids,
      columns:
        "id,scan_session_id,collection_item_id,pregrade_score,pregrade_basis,evidence_profile,outcome_risks,condition_score,condition_low,condition_high,subscores,confidence,model_bundle_version,estimate_status,confirmed_at,created_at",
      order: "created_at",
    }),
...
export async function loadGradingReports(client, collectionItemId) {
  ...
  for (const result of [predictionsResult, evidenceResult, outcomesResult])
    if (result.error) throw result.error;
