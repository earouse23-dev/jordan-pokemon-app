...
export function collectionToCsv(items) {
  const headers = ['provider_card_id','name','set','set_id','number','language','variant','card_state','condition','raw_condition','grading_company','grade','certification_number','quantity','purchase_date','purchase_price_each','total_acquisition_cost','currency','market_reference','tags','storage_location','notes'];
  const rows = items.map(item => {
    const quantity=Number(item.quantity)||0;
    const totalCost=item.costBasis!==null&&item.costBasis!==undefined&&item.costBasis!==''?Number(item.costBasis):item.cost!==null&&item.cost!==undefined&&item.cost!==''?Number(item.cost)*quantity:'';
    return [item.id,item.name,item.set,item.setId,item.number,item.language||'en',item.variant,item.cardState||(item.gradingCompany?'graded':'raw'),item.condition,item.rawCondition,item.gradingCompany,item.grade,item.certificationNumber,item.quantity,item.purchaseDate,item.cost,totalCost,item.currency||'USD',item.price,(item.tags || []).join('|'),item.location,item.notes].map(safeCsvCell).join(',');
  });
...
export function transactionReportCsv(
  ...
  const headers=['date','type','card','set','number','quantity','currency','unit_amount','total_acquisition_cost','gross_sale','net_proceeds','fifo_sold_basis','realized_profit','marketplace'];
  const rows=positions.flatMap(position=>(position.transactions||[]).map(transaction=>({position,transaction})))
    .filter(({position,transaction})=>transaction.date>=from&&transaction.date<=to&&(transaction.currency||position.currency||'USD')===currency)
    .sort((left,right)=>left.transaction.date.localeCompare(right.transaction.date)||String(left.position.name||'').localeCompare(String(right.position.name||'')))
    .map(({position,transaction})=>{
      const sale=transaction.type==='sale';
      const realized=sale&&transaction.allocatedCost!==null&&transaction.allocatedCost!==undefined&&transaction.netProceeds!==null&&transaction.netProceeds!==undefined?Number(transaction.netProceeds)-Number(transaction.allocatedCost):'';
      return [transaction.date,transaction.type,position.name,position.set,position.number,transaction.quantity,transaction.currency||position.currency||currency,transaction.unitPrice??'',sale?'':transaction.totalCost??'',sale?transaction.subtotal??'':'',sale?transaction.netProceeds??'':'',sale?transaction.allocatedCost??'':'',realized,transaction.marketplace||''].map(safeCsvCell).join(',');
    });
  return [headers.join(','),...rows].join('\r\n');
}
...
export function parseCollectionCsv(text, limit=5000) {
  ...
  rows.slice(1,limit+1).forEach((values,rowIndex)=>{
    ...
    const quantity=Number(source.quantity);const costSource=source.purchase_price_each??source.purchase_price??'';const cost=costSource===''?null:Number(costSource);const totalSource=source.total_acquisition_cost??'';const totalAcquisitionCost=totalSource===''?null:Number(totalSource);const price=source.market_reference===''||source.market_reference===undefined?null:Number(source.market_reference);
    ...
    if(totalAcquisitionCost!==null&&(!Number.isFinite(totalAcquisitionCost)||totalAcquisitionCost<0)){errors.push(`Row ${rowIndex+2}: total acquisition cost is invalid`);return;}
    if(price!==null&&(!Number.isFinite(price)||price<0)){errors.push(`Row ${rowIndex+2}: market reference is invalid`);return;}
    const record={name:source.name,set:source.set||'',number:source.number||'',variant:source.variant||'Unknown',condition:source.condition||'Near Mint',gradingCompany:source.grading_company||'',grade:source.grade||'',quantity,cost,price,tags:(source.tags||'').split('|').map(value=>value.trim()).filter(Boolean),location:source.storage_location||'',notes:source.notes||''};
    if(source.provider_card_id)record.id=source.provider_card_id;
    if(source.set_id)record.setId=source.set_id;
    if(source.language)record.language=source.language;
    if(source.card_state)record.cardState=source.card_state;
    if(source.raw_condition)record.rawCondition=source.raw_condition;
    if(source.certification_number)record.certificationNumber=source.certification_number;
    if(source.purchase_date)record.purchaseDate=source.purchase_date;
    if(totalAcquisitionCost!==null)record.totalAcquisitionCost=totalAcquisitionCost;
    if(source.currency)record.currency=source.currency.toUpperCase();
    records.push(record);
  });
...
export function matchesSearch(item, query) {
  ...
  if (!q) return true;
  const haystack = normalizeSearch([item.name,item.set,item.number,item.variant,item.rarity,item.condition,item.rawCondition,item.gradingCompany,item.grade,item.purchaseDate,item.certificationNumber,item.location,...(item.tags||[])].join(' '));
