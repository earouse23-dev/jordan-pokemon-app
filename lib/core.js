...
export function parseCollectionCsv(text, limit = 5000) {
  ...
  const missing = ["name", "quantity"].filter(
  ...
  rows.slice(1, limit + 1).forEach((values, rowIndex) => {
    ...
    const costSource =
      source.purchase_price_each ?? source.purchase_price ?? "";
    const cost = costSource === "" ? null : Number(costSource);
    const totalSource = source.total_acquisition_cost ?? "";
    const totalAcquisitionCost =
      totalSource === "" ? null : Number(totalSource);
    const price =
      source.market_reference === "" || source.market_reference === undefined
        ? null
        : Number(source.market_reference);
    if (!source.name) {
    ...
    const record = {
      ...
      variant: source.variant || "Unknown",
      condition: source.condition || "Near Mint",
      gradingCompany: source.grading_company || "",
      grade: source.grade || "",
      quantity,
      cost,
      price,
