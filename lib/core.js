...
export function selectedInventoryShare(
  ...
  textRows.forEach((row, index) => {
    ...
    const price =
      priceMode === "showcase"
        ? ""
        : ` · ${row.priceKind}: ${row.priceEach === null ? "Not available" : `${money(row.priceEach, row.currency)} each${priceMode === "market" ? ` · ${row.referenceProvider}${row.referenceObservedAt ? ` · observed ${row.referenceObservedAt}` : ""}` : ""}`}`;
...
export function parseCollectionCsv(text, limit = 5000) {
  ...
  const headerFor = (key) =>
    aliases[key].find((name) => headers.includes(name));
