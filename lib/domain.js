import { PRICE_FRESHNESS_POLICY, priceFreshness } from "./pricing.js";
...
const GRADER_CERTIFICATION_LOOKUPS = Object.freeze({
  PSA: Object.freeze({
    graderName: "PSA",
    lookupUrl: "https://www.psacard.com/cert",
    expectedFormat: "numbers printed beneath the barcode",
  }),
  BGS: Object.freeze({
    graderName: "Beckett",
    lookupUrl: "https://www.beckett.com/grading/card-lookup?flag=1",
    expectedFormat: "the certification number on the Beckett label",
  }),
  CGC: Object.freeze({
    graderName: "CGC Cards",
    lookupUrl: "https://www.cgccards.com/certlookup/",
    expectedFormat: "numbers printed beside the CGC barcode",
  }),
  TAG: Object.freeze({
    graderName: "TAG",
    lookupUrl: "https://taggrading.com/pages/cert-search",
    expectedFormat: "typically one letter followed by seven numbers",
  }),
  SGC: Object.freeze({
    graderName: "SGC",
...
export function graderCertificationLookup(grader, certificationNumber) {
  const normalizedGrader = normalizeGrader(grader).normalized;
  const provider = GRADER_CERTIFICATION_LOOKUPS[normalizedGrader] || null;
  const certification = String(certificationNumber ?? "")
    .trim()
    .slice(0, 120);
  const compact = certification.replace(/\s+/g, "");
  let directUrl = null;
  let formatRecognized = false;
  if (provider && compact) {
    if (normalizedGrader === "PSA" && /^\d{5,12}$/.test(compact)) {
      directUrl = `https://www.psacard.com/cert/${compact}`;
      formatRecognized = true;
    } else if (normalizedGrader === "CGC" && /^\d{7,20}$/.test(compact)) {
      directUrl = `https://www.cgccards.com/certlookup/${compact}/`;
      formatRecognized = true;
    } else if (normalizedGrader === "TAG" && /^[A-Z0-9]{8}$/i.test(compact)) {
      directUrl = `https://tagd.co/${compact.toUpperCase()}`;
      formatRecognized = true;
    } else if (normalizedGrader === "BGS") {
      formatRecognized = /^[A-Z0-9-]{5,24}$/i.test(compact);
    } else if (normalizedGrader === "SGC") {
      formatRecognized =
        /^\d{7}$/.test(compact) || /^\d{7}-[A-Z0-9]{3}$/i.test(compact);
    }
  }
  return {
    grader: normalizedGrader,
    graderName: provider?.graderName || String(grader || "Grader").trim(),
    certification,
    lookupUrl: directUrl || provider?.lookupUrl || null,
    direct: Boolean(directUrl),
    supported: Boolean(provider),
    formatRecognized,
    expectedFormat:
      provider?.expectedFormat || "the identifier on the slab label",
  };
}
...
export function selectValuation(observations, position, options = {}) {
  ...
  const legacyLiveHours = Number(options.staleAfterHours);
  const policy = Number.isFinite(legacyLiveHours)
    ? {
        ...PRICE_FRESHNESS_POLICY,
        market_index: {
          liveHours: legacyLiveHours,
          staleHours: legacyLiveHours,
        },
      }
    : PRICE_FRESHNESS_POLICY;
  const freshness = priceFreshness(selected, { now, policy });
  return {
    selected,
    compatible,
    stale: freshness.status !== "live",
    freshness,
    reason: freshness.status === "live" ? "exact_match" : "exact_match_stale",
  };
}
