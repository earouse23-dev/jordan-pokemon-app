...
export function selectReferenceQuote(quotes, variant, currency = 'USD', context = {}) {
  ...
  if (context.gradingCompany) {
    ...
    return compatible.find(quote => String(quote.gradingCompany || '').toUpperCase() === company && String(quote.grade ?? '') === grade) || null;
  }
  for (const provider of ['justtcg', 'tcgplayer']) {
    ...
    for (const priceType of ['market', 'mid', 'low']) {
      if (context.condition && context.condition !== 'Graded') {
        const exact = fromProvider.find(candidate => candidate.priceType === priceType && candidate.condition === context.condition);
        if (exact) return exact;
        const conditionNeutral = fromProvider.find(candidate => candidate.priceType === priceType && candidate.condition == null);
...
export function selectCardmarketReference(quotes, variant) {
  const finish = finishForVariant(variant);
  const requestedFinish = finish === 'normal' || finish === '1stEditionNormal' ? 'normal' : 'holofoil';
...
export function mergePriceHistory(...sources) {
  ...
  for (const point of sources.flat()) {
    ...
    const timestamp = point?.recordedAt ? new Date(point.recordedAt).getTime() : NaN;
    ...
    const key = [point.provider, point.providerVariantId, point.currency, point.condition, point.finish, recordedAt, amount].join('|');
    unique.set(key, normalized);
  }
  return [...unique.values()].sort((left, right) => new Date(left.recordedAt) - new Date(right.recordedAt));
}
