-- Store only a compact, versioned ordinal calibration artifact. Raw training
-- evidence remains in the isolated research tables and is never returned to
-- the product runtime.
...
alter table grading_private.calibration_registry
  ...
  add constraint calibration_registry_activation_check check (
    not active or (
      ...
      and artifact->>'version'=calibration_version
      and artifact->>'featureVersion'='mica-psa-features-v1'
      and artifact->>'validated'='true'
...
begin
  ...
  if not candidate.validated
    or candidate.artifact->>'validated'<>'true'
    or candidate.artifact->>'version'<>candidate.calibration_version
    or candidate.artifact->>'featureVersion'<>'mica-psa-features-v1'
...
begin
  ...
  if p_calibration_version is null
    ...
    or p_artifact->>'validated'<>'true'
    or p_artifact->>'version'<>p_calibration_version
    or p_artifact->>'featureVersion'<>'mica-psa-features-v1'
