-- Preserve bounded independent-review agreement metadata with each private
-- report. Never stores prompts, raw provider output, or card images.
alter table public.grading_predictions
  add column if not exists review_consensus jsonb not null default '{}'::jsonb;

alter table public.grading_predictions
  drop constraint if exists grading_predictions_review_consensus_object;
alter table public.grading_predictions
  add constraint grading_predictions_review_consensus_object
  check (jsonb_typeof(review_consensus) = 'object');
...
begin
  ...
  if jsonb_typeof(coalesce(p_capture_metadata,'[]'::jsonb))<>'array'
    ...
    or jsonb_typeof(coalesce(p_prediction->'consensus','{}'::jsonb))<>'object'
  ...
  on conflict (scan_session_id) do update set
    ...
    review_consensus=excluded.review_consensus,
