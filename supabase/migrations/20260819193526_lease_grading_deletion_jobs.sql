-- Processing jobs are leased so a crashed worker cannot strand an erasure
-- request forever. A stale lease becomes eligible for an idempotent retry.
alter table grading_private.data_deletion_jobs
...
create index if not exists deletion_jobs_claim_idx
  on grading_private.data_deletion_jobs(status,claimed_at,created_at)
  where attempts<20;
...
begin
  ...
  with selected as (
    ...
    returning job.id,job.subject_hash,job.storage_paths,job.attempts,job.claimed_at
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'leaseExpiresAt',claimed.claimed_at+interval '15 minutes'
