...
- Authentication: Supabase schema existed, but the shipped client still used local preview storage. This implementation replaces that path with Supabase email/password authentication and password recovery.
...
- Email/password is the sign-in method; existing users can create or reset a password from the same screen.
...
3. Add password auth and recovery, authenticated persistence, and transactional RPCs for position creation and FIFO sales.
