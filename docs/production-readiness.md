...
- [x] Single-purpose-token multilingual catalog importer deployed; Vault-backed resumable scheduler and hourly daily-metric rollup active.
...
- [ ] Completed catalog backfill, PkmnPrices Pro-or-higher sold entitlement, server-side key configuration, and health/circuit alerting.
- [x] Private OCR/vision benchmark runner and fail-closed quality gate.
- [x] Aggregate-only AI portfolio explanation with owner-scoped rate limiting
      and a deterministic no-AI fallback.
...
- [x] Automated desktop/mobile, theme, workspace, route, touch-target, overflow,
      and WCAG shell regression in CI.
- [ ] Physical-device camera, offline/reconnect, screen-reader, zoom, and native
...
The public project has the versioned catalog-ingestion migration, active `sync-catalog` Edge Function, and internal rollup schedule. Its scheduler credential is generated inside Postgres, encrypted in Vault, and usable only for catalog dispatch; there is no owner credential-copy step. Verify the first import and recurring invocation using [the catalog sync runbook](catalog-sync-runbook.md). Do not expose the dispatch token or replace it with a reusable service-role credential.
